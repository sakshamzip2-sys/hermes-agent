...
- 需要逐步执行某个函数并观察集合的变化
- 长期运行的进程（opencomputer gateway、tui_gateway）出现异常且无法重启
...
opencomputer 测试运行器和 pytest 均支持以下方式：
...
适用于长期运行的进程：OpenComputer gateway、tui_gateway、daemon，或已出现异常且无法干净重启的进程。
...
最简便的终端侧 DAP 客户端是 VS Code CLI 或一个小脚本。在 OpenComputer 内部有两个实用选项：
...
## 调试 OpenComputer 特定进程

### 测试
参见方案 3。始终添加 `-p no:xdist` 或在不使用 xdist 的情况下运行单个测试。

### `run_agent.py` / CLI — 一次性运行
最简单：在可疑行附近添加 `breakpoint()`，然后正常运行 `opencomputer`。控制权将在暂停点返回到你的终端。

### `tui_gateway` 子进程（由 `opencomputer --tui` 启动）
...
debugpy.wait_for_client()
```
启动 `opencomputer --tui`。TUI 将显示为冻结状态（其后端正在等待）。附加客户端后，执行在你 `continue` 时恢复。
...
9. **fork / 多进程。** pdb 不会跟随 fork。每个子进程需要自己的 `breakpoint()` 或 `set_trace()`。对于 OpenComputer 子 agent，每次只调试一个进程。
