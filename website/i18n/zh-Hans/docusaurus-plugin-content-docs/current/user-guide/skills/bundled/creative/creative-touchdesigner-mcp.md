...
| 相关 skill | [`native-mcp`](/user-guide/skills/bundled/mcp/mcp-native-mcp), [`ascii-video`](/user-guide/skills/bundled/creative/creative-ascii-video), [`manim-video`](/user-guide/skills/bundled/creative/creative-manim-video), `opencomputer-video` |
...
```
OpenComputer -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. 将 `twozero_td` MCP 服务器添加到 OpenComputer 配置（如果缺失）
...
3. **重启 OpenComputer 会话**以加载新的 MCP 服务器
