...
sidebar_position: 4
title: "贡献指南"
description: "如何为 OpenComputer 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 OpenComputer 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 OpenComputer 核心？从 [构建 OpenComputer 插件](../guides/build-a-opencomputer-plugin.md) 开始
- 为 OpenComputer 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
OpenComputer 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
2. **手动测试**：运行 `opencomputer` 并验证您修改的代码路径
...
- 请包含：操作系统、Python 版本、OpenComputer 版本（`opencomputer version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
