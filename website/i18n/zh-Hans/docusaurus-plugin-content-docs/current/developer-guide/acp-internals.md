...
ACP 适配器将 OpenComputer 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
opencomputer acp / hermes-acp / python -m acp_adapter
  ...
  -> construct OpenComputerACPAgent
...
- 拒绝选项 -> OpenComputer `deny`

超时和桥接失败默认拒绝。

### 工具渲染辅助

`acp_adapter/tools.py` 将 OpenComputer 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 OpenComputer 的运行时解析器：

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 OpenComputer provider/凭据。它还始终通告一个终端 setup 认证方法（`opencomputer-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 OpenComputer 的交互式模型/provider 配置。
...
- `hermes_cli/main.py` — `opencomputer acp` CLI 子命令
