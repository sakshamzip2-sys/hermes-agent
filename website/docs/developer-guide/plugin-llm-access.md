...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept OpenComputer already has applies. The plugin never sees a
...
[`opencomputer-example-plugins`](https://github.com/NousResearch/opencomputer-example-plugins/tree/main/plugin-llm-example)
...
[`plugin-llm-async-example`](https://github.com/NousResearch/opencomputer-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — OpenComputer provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different OpenComputer provider
        # (must be one OpenComputer already knows about — same names as
        # `opencomputer model` and config.yaml model.provider).
        ...
        # plugin sends — OpenComputer does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through OpenComputer' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing OpenComputer subsystem:
...
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/opencomputer-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/opencomputer-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
