...
description: "How to build an image-generation backend plugin for OpenComputer"
---
...
Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `opencomputer tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `opencomputer tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.hermes/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `opencomputer plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `opencomputer tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `opencomputer tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `opencomputer tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `opencomputer plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `opencomputer tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `opencomputer tools` model picker |
...
Drop a user plugin at `~/.hermes/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `opencomputer plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export HERMES_HOME=/tmp/opencomputer-imggen-test
...
opencomputer plugins enable my-backend
...
# Exercise it
opencomputer -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `opencomputer tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-opencomputer-plugin#distribute-via-pip) in the general plugin guide for the full setup.
