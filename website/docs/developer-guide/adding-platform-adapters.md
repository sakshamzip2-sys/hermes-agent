...
This guide covers adding a new messaging platform to the OpenComputer gateway. A platform adapter connects OpenComputer to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core OpenComputer code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `opencomputer config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-opencomputer-config) below).
...
| `opencomputer config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `opencomputer status` | Shows plugin platforms with `(plugin)` tag |
| `opencomputer gateway setup` | Plugin platforms appear in setup menu |
| `opencomputer tools` / `opencomputer skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `opencomputer gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `opencomputer cron run` separate from `opencomputer gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `opencomputer config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for OpenComputer.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #opencomputer)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the OpenComputer core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`toolsets.py`** — Add `"opencomputer-newplat"` toolset definition with `_HERMES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"opencomputer-newplat"` to the `"hermes-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | opencomputer-newplat toolset |
