...
sidebar_position: 1
title: "Architecture"
description: "OpenComputer internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of OpenComputer internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│   ├── main.py               # Entry point — all `opencomputer` subcommands (large file)
...
│   ├── skills_config.py      # opencomputer skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # opencomputer tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # opencomputer gateway start/stop
│
...
Three discovery sources: `~/.hermes/plugins/` (user), `.hermes/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `opencomputer plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-opencomputer-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes OpenComputer as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`opencomputer -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
