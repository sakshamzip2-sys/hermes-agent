...
The messaging gateway is the long-running process that connects OpenComputer to 20+ external messaging platforms through a unified architecture.
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`opencomputer send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
- `opencomputer gateway start` / `opencomputer gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.hermes/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `opencomputer gateway stop` stops only the current profile's gateway. `opencomputer gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
