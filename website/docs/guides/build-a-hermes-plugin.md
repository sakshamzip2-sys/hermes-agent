...
title: "Build a OpenComputer Plugin"
description: "Step-by-step guide to building a complete OpenComputer plugin with tools, hooks, data files, and skills"
---

# Build a OpenComputer Plugin

This guide walks through building a complete OpenComputer plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
OpenComputer has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `opencomputer skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
This tells OpenComputer: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** OpenComputer may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `opencomputer my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but OpenComputer continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 opencomputer plugins list
```
...
```bash
opencomputer logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `opencomputer plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
...
When users run `opencomputer plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing OpenComputer into installing arbitrary packages — only specs OpenComputer itself ships are eligible |
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), OpenComputer injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is OpenComputer's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `opencomputer <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for opencomputer my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: opencomputer my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for opencomputer my-plugin."""
...
After registration, users can run `opencomputer my-plugin status`, `opencomputer my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `opencomputer name` in a terminal |
...
OpenComputer has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hermes/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `opencomputer gateway status` without SDK instantiation.
...
OpenComputer also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into OpenComputer without any Python plugin. Declare them in `~/.hermes/config.yaml`:
...
OpenComputer connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).
...
opencomputer skills search my-workflow --source myorg/skills-repo
opencomputer skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install opencomputer-plugin-calculator
# Plugin auto-discovered on next opencomputer startup
```
...
```python
# Wrong — will break if OpenComputer passes extra context
def handler(args):
    ...
