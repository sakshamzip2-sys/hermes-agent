...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `opencomputer send`."
---

# Pipe Script Output to Messaging Platforms

`opencomputer send` is a small, scriptable CLI that pushes a message to any
messaging platform OpenComputer is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | opencomputer send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `opencomputer
...
opencomputer send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
opencomputer send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
opencomputer send --list

# Filter by platform
opencomputer send --list telegram
```
...
Any platform OpenComputer ships adapters for works as a target:
...
`opencomputer send` resolves the message body in this order:

1. **Positional argument** — `opencomputer send --to telegram "hi"`
2. **`--file PATH`** — `opencomputer send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | opencomputer send --to telegram`

When stdin is a TTY (no pipe), OpenComputer does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  opencomputer send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `opencomputer send` reuses your OpenComputer config, the same script works on
any host where OpenComputer is installed — no need to export bot tokens into
...
`opencomputer send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  opencomputer send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | opencomputer send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.hermes/bin/opencomputer send \
...
```bash
./train.py --epochs 200 && \
  opencomputer send --to telegram "training done" || \
  opencomputer send --to telegram "training failed (exit $?)"
```
...
opencomputer send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(opencomputer send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `opencomputer send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `opencomputer send` calls
...
pointing at the gateway; start it with `opencomputer gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`opencomputer gateway
...
| Approach | Multi-platform | Reuses OpenComputer creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `opencomputer send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`opencomputer send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `opencomputer send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `opencomputer send` shares with cron delivery.
