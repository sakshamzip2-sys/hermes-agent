---
sidebar_position: 6
title: "Use MCP with OpenComputer"
description: "A practical guide to connecting MCP servers to OpenComputer, filtering their tools, and using them safely in real workflows"
---

# Use MCP with OpenComputer

This guide shows how to actually use MCP with OpenComputer in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native OpenComputer tool
- you want OpenComputer to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect OpenComputer to internal APIs, databases, or company systems without modifying OpenComputer core

Do not use MCP when:
- a built-in OpenComputer tool already solves the job well
...
- OpenComputer remains the agent
- MCP servers contribute tools
- OpenComputer discovers those tools at startup or reload time
...
If you installed OpenComputer with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- OpenComputer banner/status should show MCP integration when configured
- ask OpenComputer what tools it has available
...
## WSL2: bridge OpenComputer in WSL to Windows Chrome

This is the practical setup when:

- OpenComputer runs inside WSL2
...
In this setup, OpenComputer does **not** connect to Chrome directly. Instead:

- OpenComputer runs in WSL
- OpenComputer starts a local stdio MCP server
...
```text
OpenComputer (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- OpenComputer stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on OpenComputer core browser transport
...
```bash
opencomputer mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
opencomputer mcp test chrome-devtools-win
```

Then start a fresh OpenComputer session or run:
...
Once loaded, OpenComputer can use the MCP-prefixed browser tools directly. For example:
...
If OpenComputer runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start OpenComputer from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start OpenComputer from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in OpenComputer:
...
2. OpenComputer-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So OpenComputer will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want OpenComputer to reason over a bounded workspace.
...
Now OpenComputer can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing OpenComputer core.
...
Because OpenComputer now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
