...
Copy-paste blueprints for common automation patterns. Each blueprint uses OpenComputer's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`opencomputer webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `opencomputer webhook subscribe`) |
...
```bash
opencomputer webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
opencomputer webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
opencomputer webhook subscribe alert-triage \
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "OpenComputer-Monitor/1.0"})
...
```bash
opencomputer cron create "every 30m" \
...
```bash
opencomputer cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to OpenComputer development." \
...
```bash
opencomputer webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
opencomputer webhook subscribe stripe-payments \
...
```bash
opencomputer cron create "0 3 * * 0" \
...
```bash
opencomputer cron create "0 10 * * 3" \
