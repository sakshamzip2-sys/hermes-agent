...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to OpenComputer — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`opencomputer claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into OpenComputer. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `opencomputer setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written before apply, under `~/.hermes/backups/pre-migration-*.zip`; restorable with `opencomputer import`). |
...
| What | OpenClaw source | OpenComputer destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | OpenComputer destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing OpenComputer skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | OpenComputer destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | OpenComputer config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | OpenComputer config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | OpenComputer field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | OpenComputer `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | OpenComputer path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct OpenComputer equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in OpenComputer |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | OpenComputer has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `opencomputer cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `opencomputer webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `opencomputer honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `opencomputer skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use OpenComputer profiles |
...
4. **Verify API keys** — run `opencomputer status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — run `opencomputer config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `opencomputer whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `opencomputer claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `opencomputer config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
