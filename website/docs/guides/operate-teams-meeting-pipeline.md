...
```bash
opencomputer teams-pipeline validate
```
...
```bash
opencomputer teams-pipeline token-health
opencomputer teams-pipeline token-health --force-refresh
```
...
```bash
opencomputer teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
opencomputer teams-pipeline maintain-subscriptions
opencomputer teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: OpenComputer cron (recommended if you already run the OpenComputer gateway)

OpenComputer ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:

```bash
mkdir -p ~/.hermes/scripts
cat > ~/.hermes/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec opencomputer teams-pipeline maintain-subscriptions
EOF
...
```ini
[Unit]
Description=OpenComputer Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=opencomputer
EnvironmentFile=/etc/opencomputer/env
ExecStart=/usr/local/bin/opencomputer teams-pipeline maintain-subscriptions
```
...
```ini
[Unit]
Description=Run OpenComputer Teams pipeline subscription maintenance every 12 hours
...
```cron
0 */12 * * * /usr/local/bin/opencomputer teams-pipeline maintain-subscriptions >> /var/log/opencomputer/teams-pipeline-maintain.log 2>&1
```
...
```bash
opencomputer teams-pipeline subscriptions   # should show expirationDateTime advanced
opencomputer teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
opencomputer teams-pipeline list
opencomputer teams-pipeline list --status failed
opencomputer teams-pipeline show <job-id>
```

### Replay a stored job

```bash
opencomputer teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
opencomputer teams-pipeline fetch --meeting-id <meeting-id>
opencomputer teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `opencomputer teams-pipeline maintain-subscriptions --dry-run`
- inspect `opencomputer teams-pipeline list --status failed`
...
- run `opencomputer teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `opencomputer teams-pipeline run`
...
- [ ] `opencomputer teams-pipeline validate` returns an OK snapshot
- [ ] `opencomputer teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (OpenComputer cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
