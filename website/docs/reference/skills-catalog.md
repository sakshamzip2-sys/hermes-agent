...
OpenComputer also syncs bundled skills on `opencomputer update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with OpenComputer; restore it with `opencomputer skills reset <name> --restore`.
...
| [`hermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) | Configure, extend, or contribute to OpenComputer. | `autonomous-ai-agents/hermes-agent` |
