...
- **Anthropic** — Claude models (direct API, OAuth via `opencomputer auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `opencomputer model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Yes, natively.** OpenComputer supports native Windows via the PowerShell installer — no WSL required. Run in PowerShell:
...
WSL2 remains a fully supported alternative. To run OpenComputer inside WSL2, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and use the standard install command:

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
```

### I run OpenComputer in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run OpenComputer inside WSL2
...
- let OpenComputer use the resulting MCP browser tools

This is more reliable than trying to force OpenComputer core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with OpenComputer](../guides/use-mcp-with-opencomputer.md#wsl2-bridge-opencomputer-in-wsl-to-windows-chrome)
...
Yes — OpenComputer now has a tested Termux install path for Android phones.
...
Yes. Run `opencomputer model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← OpenComputer minimum; set this to match your server's actual context window
```
...
OpenComputer persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in OpenComputer — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
Yes. Import the `AIAgent` class and use OpenComputer programmatically:
...
#### `opencomputer: command not found` after installation
...
**Cause:** OpenComputer requires Python 3.11 or newer.
...
**Cause:** OpenComputer builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when OpenComputer runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** OpenComputer auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
sudo rm /usr/local/bin/opencomputer
...
**Solution:** Exit your session and use `opencomputer model` from your terminal to add new providers:

```bash
# Exit the OpenComputer chat session first (Ctrl+C or /quit)
...
After adding a new provider via `opencomputer model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `opencomputer model` (from terminal) |
| Enter/change API keys | `opencomputer model` (from terminal) |
...
opencomputer config show

# Re-configure your provider
opencomputer model

# Or set directly
opencomputer config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
opencomputer config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
opencomputer chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `opencomputer chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or OpenComputer detected the wrong context length for your model.
...
opencomputer chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, OpenComputer may have the wrong context length for your model. Check what it detected:
...
**Cause:** OpenComputer detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — OpenComputer never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `opencomputer chat`
...
**Solution:**
- Verify your bot token is valid with `opencomputer gateway setup`
...
#### WSL: Gateway keeps disconnecting or `opencomputer gateway start` fails
...
tmux new -s opencomputer 'opencomputer gateway run'
# Reattach later: tmux attach -t opencomputer

# Option 3: Background via nohup
nohup opencomputer gateway run > ~/.hermes/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'opencomputer gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `opencomputer gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
opencomputer gateway install    # Re-snapshots your current PATH
opencomputer gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `opencomputer chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `opencomputer chat -t "terminal"`
...
opencomputer config show | grep -A 12 mcp_servers

# Restart OpenComputer or reload MCP after config changes
opencomputer chat
```
...
:::warning
If an MCP server crashes mid-request, OpenComputer will report a timeout. Check the server's own logs (not just OpenComputer logs) to diagnose the root cause.
:::
...
`opencomputer update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `opencomputer update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** OpenComputer can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell OpenComputer "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can OpenComputer do this?

**Current limitation:** OpenComputer profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, OpenComputer reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `opencomputer skills config` settings don't seem to take effect.

**Solution:** Use `opencomputer skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`opencomputer gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** OpenComputer creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting OpenComputer to another machine
...
2. On the **source machine**, create a full backup:
   ...
   opencomputer backup
   ```
   This creates a zip of your entire `~/.hermes/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/opencomputer-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/opencomputer-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   opencomputer import ~/opencomputer-backup-<timestamp>.zip
   ```

4. On the new machine, run `opencomputer setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
opencomputer profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
opencomputer profile import ./work-backup.tar.gz work
```
...
### `opencomputer backup` vs `opencomputer profile export`

| Feature | `opencomputer backup` | `opencomputer profile export` |
| :--- | :--- | :--- |
...
:::tip
`opencomputer backup` produces a consistent snapshot even while OpenComputer is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the OpenComputer installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a OpenComputer-specific issue — it's a shell config permissions problem.
...
opencomputer config show | head -20

# Re-run model selection
opencomputer model

# Or test with a known-good model
opencomputer chat -q "hello" --model anthropic/claude-opus-4.7
```
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), OpenComputer version (`opencomputer --version`), and the full error message
