...
description: "Reference for OpenComputer MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
OpenComputer may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, OpenComputer only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, OpenComputer does not create an empty MCP runtime toolset for that server.
...
Behavior:
- OpenComputer uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
