...
This page covers all commands related to [OpenComputer profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `opencomputer profile` without a subcommand shows help.
...
## `opencomputer profile use`

```bash
opencomputer profile use <name>
```

Sets `<name>` as the active profile. All subsequent `opencomputer` commands (without `-p`) will use this profile.
...
opencomputer profile use default
```

## `opencomputer profile create`

```bash
opencomputer profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `opencomputer profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `opencomputer update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.hermes`), use `opencomputer skills opt-out` / `opencomputer skills opt-in`. |
...
opencomputer profile create mybot

# Clone config only from current profile
opencomputer profile create work --clone

# Clone everything from current profile
opencomputer profile create backup --clone-all

# Clone config from a specific profile
opencomputer profile create work2 --clone-from work

# Clone everything from a specific profile
opencomputer profile create work2-backup --clone-from work --clone-all
```

## `opencomputer profile describe`

```bash
opencomputer profile describe [<name>] [options]
```
...
opencomputer profile describe researcher

# Set it explicitly
opencomputer profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
opencomputer profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
opencomputer profile describe --all --auto
```

## `opencomputer profile delete`

```bash
opencomputer profile delete <name> [options]
```
...
```bash
opencomputer profile delete mybot
opencomputer profile delete mybot --yes
```
...
## `opencomputer profile show`

```bash
opencomputer profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's OpenComputer home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ opencomputer profile show work
Profile: work
...
## `opencomputer profile alias`

```bash
opencomputer profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your OpenComputer installation.
...
```bash
opencomputer profile alias work
# Creates/updates ~/.local/bin/work

opencomputer profile alias work --name mywork
# Creates ~/.local/bin/mywork

opencomputer profile alias work --remove
# Removes the wrapper script
```

## `opencomputer profile rename`

```bash
opencomputer profile rename <old-name> <new-name>
```
...
```bash
opencomputer profile rename mybot assistant
# ~/.hermes/profiles/mybot → ~/.hermes/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `opencomputer profile export`

```bash
opencomputer profile export <name> [options]
```
...
```bash
opencomputer profile export work
# Creates work.tar.gz in the current directory

opencomputer profile export work -o ./work-2026-03-29.tar.gz
```

## `opencomputer profile import`

```bash
opencomputer profile import <archive> [options]
```
...
```bash
opencomputer profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

opencomputer profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`opencomputer profile export` / `import` are still the right commands for
...
### `opencomputer profile install`

```bash
opencomputer profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `opencomputer -p telemetry`). |
...
opencomputer profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
opencomputer profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
opencomputer profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
opencomputer profile install ./telemetry/
```

### `opencomputer profile update`

```bash
opencomputer profile update <name> [--force-config] [--yes]
```
...
### `opencomputer profile info`

```bash
opencomputer profile info <name>
```

Prints the profile's distribution manifest — name, version, required
OpenComputer version, author, env var requirements, the source URL/path, and
...
`opencomputer profile list` also shows the distribution name and version in a
`Distribution` column, and `opencomputer profile show <name>` / `delete <name>`
...
opencomputer profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
opencomputer profile install https://github.com/your-org/internal-assistant.git
```
...
OpenComputer version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host OpenComputer can clone from.
3. Tell recipients to run `opencomputer profile install <your-repo-url>`.
...
## `opencomputer -p` / `opencomputer --profile`

```bash
opencomputer -p <name> <command> [options]
opencomputer --profile <name> <command> [options]
```

Global flag to run any OpenComputer command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
opencomputer -p work chat -q "Check the server status"
opencomputer --profile dev gateway start
opencomputer -p personal skills list
opencomputer -p work config edit
```

## `opencomputer completion`

```bash
opencomputer completion <shell>
```
...
- `opencomputer profile <TAB>` — subcommands (list, use, create, etc.)
- `opencomputer profile use <TAB>` — profile names
- `opencomputer -p <TAB>` — profile names
