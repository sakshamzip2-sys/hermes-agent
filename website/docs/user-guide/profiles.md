...
Run multiple independent OpenComputers on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.
...
```bash
opencomputer profile create work --clone --clone-from coder
```
