...
Run multiple independent OpenComputer agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate OpenComputer home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up OpenComputer state.
...
```bash
opencomputer profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own OpenComputer profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `opencomputer setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
opencomputer profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `opencomputer profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
opencomputer profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every opencomputer subcommand — it's just `opencomputer -p <name>` under the hood.
...
```bash
opencomputer -p coder chat
opencomputer --profile=coder doctor
opencomputer chat -p coder -q "hello"    # works in any position
```

### Sticky default (`opencomputer profile use`)

```bash
opencomputer profile use coder
opencomputer chat                   # now targets coder
opencomputer tools                  # configures coder's tools
opencomputer profile use default    # switch back
```

Sets a default so plain `opencomputer` commands target that profile. Like `kubectl config use-context`.
...
- **`opencomputer profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives OpenComputer its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory OpenComputer was launched from", not "the profile directory".
...
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `opencomputer profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `opencomputer -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
default for **future CLI/gateway runs** (same as `opencomputer profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`opencomputer update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
opencomputer profile list           # show all profiles with status
opencomputer profile show coder     # detailed info for one profile
opencomputer profile rename coder dev-bot   # rename (updates alias + service)
opencomputer profile export coder   # export to coder.tar.gz
opencomputer profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
opencomputer profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `opencomputer profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.hermes`). To remove everything, use `opencomputer uninstall`.
:::
...
```bash
# Bash
eval "$(opencomputer completion bash)"

# Zsh
eval "$(opencomputer completion zsh)"
```
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.hermes/profiles/coder` before launching opencomputer. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, OpenComputer state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls OpenComputer config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other OpenComputer
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, OpenComputer keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode OpenComputer launches tool
...
OpenComputer also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
