...
- A long-running process (opencomputer gateway, tui_gateway) misbehaves and you can't restart it
...
The opencomputer test runner and pytest both support this:
...
For long-lived processes: OpenComputer gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside OpenComputer you have two practical options:
...
## Debugging OpenComputer-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `opencomputer` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `opencomputer --tui`)
...
debugpy.wait_for_client()
```
Start `opencomputer --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For OpenComputer subagents, debug one process at a time.
