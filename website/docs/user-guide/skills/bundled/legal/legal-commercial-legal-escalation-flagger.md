---
title: "Commercial Legal Escalation Flagger — Route a contract issue to the right approver per the escalation matrix in `~/"
sidebar_label: "Commercial Legal Escalation Flagger"
description: "Route a contract issue to the right approver per the escalation matrix in `~/"
---
...
Route a contract issue to the right approver per the escalation matrix in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`, and draft the ask. Use when the user says "who needs to approve this", "escalate this", "does this need GC sign-off", "route this for approval", or when another skill finds an issue that exceeds the reviewer's authority.
...
| Path | `skills/legal/commercial-legal/escalation-flagger` |
...
Names the approver for a contract issue per the `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` escalation matrix and drafts the message so you're not writing "hey got a sec" at 5pm.

## Instructions

1. **Load `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`** → Escalation section. If missing, say so -- the practice profile needs editing.
...
Every contracts team has an escalation matrix, written or not. This skill reads the written one (in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`), matches a contract issue against it, names the approver, and drafts the ask so the lawyer isn't writing "hey do you have a sec" messages at 5pm.
...
Read `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` → `## Escalation`. If it's missing or vague, say so -- the cold-start interview should have captured this, and if it didn't, the practice profile needs editing.
...
Don't escalate things that are actually fine. If the term is within the fallbacks in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`, it doesn't need to go up.
...
Be specific. Not "escalate to legal leadership" -- name the person or role from `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`. If the matrix doesn't name anyone for this situation, say so: "The escalation matrix doesn't cover [situation]. Suggest asking [GC name] who owns this."
...
```markdown
**Escalating to:** [name]
**Via:** [Slack #channel / email / meeting -- per `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`]
...
**What our playbook says:** [quote from `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`]
...
The calibration for what warrants escalation lives in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`, not in this skill. Check the playbook's stated position, its fallbacks, and its "automatic escalation regardless of dollar value" list:
...
If a term comes up that the playbook doesn't address, don't guess the threshold -- ask the reviewing attorney whether this class of issue should escalate, and offer to record the answer in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` so future reviews are consistent.
