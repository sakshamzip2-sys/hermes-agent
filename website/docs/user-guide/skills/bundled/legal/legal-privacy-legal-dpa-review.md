---
title: "Privacy Legal Dpa Review"
sidebar_label: "Privacy Legal Dpa Review"
...
| Path | `skills/legal/privacy-legal/dpa-review` |
...
1. Load `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → DPA playbook. If placeholders, stop and prompt setup.
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → `## DPA playbook`. Also read `## Privacy policy commitments` -- the DPA can't contradict what the privacy policy promises.
...
Prepend the work-product header from `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role -- see `## Who's using this`).
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
