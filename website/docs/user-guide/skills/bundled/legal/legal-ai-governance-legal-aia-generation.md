---
title: "Ai Governance Legal Aia Generation"
sidebar_label: "Ai Governance Legal Aia Generation"
description: "Run an AI impact assessment -- structured intake, risk analysis, regulatory classification per regime in scope, policy consistency diff, and recommendation w..."
---
...
Run an AI impact assessment -- structured intake, risk analysis, regulatory classification per regime in scope, policy consistency diff, and recommendation with conditions. Uses the house-style structure learned from the seed impact assessment in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`. Use when user says "impact assessment for", "assess this AI use case", "run an AIA", "generate an AIA", "we need to document this AI system", "AI risk assessment for X", or follows a conditional triage result.
...
| Path | `skills/legal/ai-governance-legal/aia-generation` |
...
1. Read `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` AI policy commitments.
...
- Is the use case in the elevated or high governance tier per `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
**Fast track vs. full assessment:** `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` defines what qualifies for abbreviated treatment. If `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` → `## Regulatory footprint` that applies to this system -- **plus any regime surfaced by the re-derivation above** -- research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
Common drift:
