---
title: "Corporate Legal Ai Tool Handoff"
sidebar_label: "Corporate Legal Ai Tool Handoff"
description: "Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust le..."
---
...
Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust level in `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md`. Use when user says "send to Luminance", "bulk review", "AI extraction", or when diligence-issue-extraction hits a high-volume category.
...
| Path | `skills/legal/corporate-legal/ai-tool-handoff` |
...
`~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` → AI-assisted review:
...
If `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` -- who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` says this -- it's rare.)
...
**Trust level:** [per `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md`]
...
- It doesn't replace the tool's output with its own judgment entirely -- if `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level -- that's in `~/.hermes/legal-practice-profile/corporate-legal/CLAUDE.md`, set at cold-start based on the team's experience with the tool.
