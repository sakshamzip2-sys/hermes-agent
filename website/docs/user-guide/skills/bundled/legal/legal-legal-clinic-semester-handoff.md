---
title: "Legal Clinic Semester Handoff — End-of-semester case handoff memos -- the mirror of /ramp"
sidebar_label: "Legal Clinic Semester Handoff"
...
| Path | `skills/legal/legal-clinic/semester-handoff` |
...
1. Load `~/.hermes/legal-practice-profile/legal-clinic/CLAUDE.md` → clinic profile, semester dates, supervision style.
2. Load `~/.hermes/legal-practice-profile/legal-clinic/deadlines.yaml` and `~/.hermes/legal-practice-profile/legal-clinic/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `~/.hermes/legal-practice-profile/legal-clinic/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `~/.hermes/legal-practice-profile/legal-clinic/handoffs/[semester]/_summary.md`.
...
- `~/.hermes/legal-practice-profile/legal-clinic/CLAUDE.md` → clinic profile, semester, practice areas, supervision style
- `~/.hermes/legal-practice-profile/legal-clinic/deadlines.yaml` → all active deadlines, grouped by case
- `~/.hermes/legal-practice-profile/legal-clinic/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `~/.hermes/legal-practice-profile/legal-clinic/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `~/.hermes/legal-practice-profile/legal-clinic/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `~/.hermes/legal-practice-profile/legal-clinic/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `~/.hermes/legal-practice-profile/legal-clinic/handoffs/[semester]/_summary.md`:
...
Closing a case or transitioning it to a new student is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.hermes/legal-practice-profile/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Case-closing memos always get professor sign-off before the case is marked closed in the handoff document, regardless of supervision-style choice.

Per `~/.hermes/legal-practice-profile/legal-clinic/CLAUDE.md` supervision style:
...
Once reviewed, handoff memos live at `~/.hermes/legal-practice-profile/legal-clinic/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `/ramp` run at the start of next semester -- `/ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`/ramp`:** at the start of next semester, reads `~/.hermes/legal-practice-profile/legal-clinic/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
