---
title: "Commercial Legal Renewal Tracker"
sidebar_label: "Commercial Legal Renewal Tracker"
...
| Path | `skills/legal/commercial-legal/renewal-tracker` |
...
1. **Read `~/.hermes/legal-practice-profile/commercial-legal/renewal-register.yaml`** (the config directory -- survives plugin updates).
...
Lives at `~/.hermes/legal-practice-profile/commercial-legal/renewal-register.yaml` (the config directory -- survives plugin updates). Each entry:
...
**Before proceeding to accept or decline a renewal (including sending a non-renewal notice or letting an auto-renewal run past the cancel-by date):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
The renewal-watcher agent in this plugin runs this skill on a schedule (weekly by default) and posts the "coming up" report to the channel named in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` → `## House style` → where work product goes. Mode 2 is the agent's primary output.
