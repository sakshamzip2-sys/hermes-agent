---
title: "Privacy Legal Dsar Response"
sidebar_label: "Privacy Legal Dsar Response"
description: "Walk through a Data Subject Access Request (or deletion, portability, correction request) and draft the response -- verify identity, locate data system-by-sy..."
---
...
| Path | `skills/legal/privacy-legal/dsar-response` |
...
1. Load `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → DSAR process (systems list, verification method, SLA).
...
Read `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → `## DSAR process`. That section has:
...
Per the method in `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md`. Common approaches:
...
Walk the systems list from `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product -- keep those separate and prepend the work-product header per `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` `## Outputs` (which differs by user role -- see `## Who's using this`).
...
Per `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → Escalation table, escalate when:
...
If `~/.hermes/legal-practice-profile/privacy-legal/CLAUDE.md` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
