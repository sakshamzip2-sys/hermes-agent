---
title: "Ip Legal Ip Clause Review"
sidebar_label: "Ip Legal Ip Clause Review"
...
| Path | `skills/legal/ip-legal/ip-clause-review` |
...
Reviews the IP clauses in an agreement against the practice profile in `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `/ip-legal:cold-start-interview` first -- I need to learn your practice profile before I can review IP clauses against it."
...
**Before reading the agreement, read `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md` → `## Outputs` (it differs by user role -- see `## Who's using this`).
