---
title: "Regulatory Legal Policy Redraft"
sidebar_label: "Regulatory Legal Policy Redraft"
...
| Path | `skills/legal/regulatory-legal/policy-redraft` |
...
1. Load `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md` → policy library index + practice profile.
...
One of:
- A `GAP-ID` from the gap tracker -- load the entry from `~/.hermes/legal-practice-profile/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
...
This skill reads the policy library index and owners from `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`. When a value it needs is empty or still `[PLACEHOLDER]`:
