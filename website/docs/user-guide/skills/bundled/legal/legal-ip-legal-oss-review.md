---
title: "Ip Legal Oss Review — Open source license compliance check for a dependency list, a single library, or outbound code"
sidebar_label: "Ip Legal Oss Review"
...
| Path | `skills/legal/ip-legal/oss-review` |
...
**Before scanning dependencies, read `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md` → `## Outputs` (differs by user role -- see `## Who's using this`).
