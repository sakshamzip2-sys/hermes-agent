---
title: "Law Student Bar Prep Questions — Bar prep questions -- MBE or essay, targeted at your weak subjects and bar jurisdiction"
sidebar_label: "Law Student Bar Prep Questions"
description: "Bar prep questions -- MBE or essay, targeted at your weak subjects and bar jurisdiction"
---
...
Bar prep questions -- MBE or essay, targeted at your weak subjects and bar jurisdiction. Tracks misses and comes back to patterns. Use when the user says "bar prep", "MBE questions", "practice essay", or "test me for the bar".
...
| Path | `skills/legal/law-student/bar-prep-questions` |
...
1. Load `~/.hermes/legal-practice-profile/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course.
2. Also load `~/.hermes/legal-practice-profile/law-student/study-plan.yaml` if it exists -- it tells you what subject is scheduled for today and what subtopics are still weak.
...
1. Load `~/.hermes/legal-practice-profile/law-student/CLAUDE.md` and read the bar jurisdiction and bar date.
...
`~/.hermes/legal-practice-profile/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course. If exam format isn't specified, run the "Exam type" gate above before continuing. If jurisdiction is specified, apply the `## Jurisdiction handling` rules -- label questions by which rule body governs, and flag divergences explicitly.

Also load `~/.hermes/legal-practice-profile/law-student/study-plan.yaml` if it exists (written by the `study-plan` skill). If the plan has a session scheduled for today or specifies weak subjects to weight, honor it.

## Session mode

`--session <n>` runs a focused N-question session on a specific subject, tracks performance, and writes session results back to `~/.hermes/legal-practice-profile/law-student/study-plan.yaml` under `session_history` so the study plan adapts.
...
If no `study-plan.yaml` exists, write session history to `~/.hermes/legal-practice-profile/law-student/session-history.yaml` instead so future sessions can still weight appropriately.
...
Subject distribution: weight toward weak subjects **within the subjects actually tested on the student's exam**. If `~/.hermes/legal-practice-profile/law-student/CLAUDE.md` says weak on Evidence and Civ Pro, 60% of questions come from those.
