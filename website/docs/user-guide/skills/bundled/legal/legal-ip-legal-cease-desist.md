---
title: "Ip Legal Cease Desist — Draft a cease-and-desist letter (send mode) or triage one you received (receive mode)"
sidebar_label: "Ip Legal Cease Desist"
...
| Path | `skills/legal/ip-legal/cease-desist` |
...
1. **Read the practice profile.** Load `~/.hermes/legal-practice-profile/ip-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This plugin needs setup before it can give you useful output. Run `/ip-legal:cold-start-interview` -- the C&D skill depends on your enforcement posture, approval matrix, and practice-area mix, none of which are configured yet."
...
- **Bypass the gate.** The send-mode gate runs every time. No bypass path is provided or honored.
