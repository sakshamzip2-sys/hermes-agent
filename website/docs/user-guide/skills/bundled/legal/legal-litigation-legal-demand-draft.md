---
title: "Litigation Legal Demand Draft"
sidebar_label: "Litigation Legal Demand Draft"
description: "Draft a demand letter from a completed intake, gated on a privilege / FRE 408 / waiver / admission checklist, with a"
---
...
| Path | `skills/legal/litigation-legal/demand-draft` |
...
1. Load `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md`; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/checklist.md` (post-send checklist).
...
- `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/intake.md` -- required; refuse to proceed if missing
- `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` → Demand-letter practice (seed-doc paths, insurance-tender timing, materiality threshold for matter creation), house style (privilege markings, outside counsel directive format for tone reference). **Tone, compliance period, marking, and signer come from `## Posture for this matter` -- they are matter-level, not practice-level.**
- `~/.hermes/legal-practice-profile/litigation-legal/matters/_log.yaml` -- to check for existing related matters (same counterparty) and offer cross-link
...
- `--skip-gate` → Not honored in this deployment. The pre-draft privilege, FRE 408, waiver, and admission checklist must run inline before any demand is drafted. A flag is not evidence the checklist was completed.
...
Check `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` privilege conventions exactly.

## Output

### Primary: `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/draft-v[N].docx`
...
### `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/[slug]/checklist.md` -- the post-send checklist
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` medium-severity band
...
> Create a tracked matter in `~/.hermes/legal-practice-profile/litigation-legal/matters/_log.yaml`? (default: [yes/no])
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `~/.hermes/legal-practice-profile/litigation-legal/demand-letters/` only.
...
- **Bypass the pre-draft gate.** `--skip-gate` is not honored; the pre-draft gate runs unconditionally and there is no bypass path.
