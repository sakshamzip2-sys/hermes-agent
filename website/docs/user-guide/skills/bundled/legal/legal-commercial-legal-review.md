---
title: "Commercial Legal Review — Review a vendor agreement, NDA, or SaaS subscription against your playbook"
sidebar_label: "Commercial Legal Review"
...
| Path | `skills/legal/commercial-legal/review` |
...
Reviews an inbound agreement against the playbook in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`. Identifies the agreement structure from titles, selects the appropriate skill(s), and -- if confirm_routing is enabled -- checks with the user before proceeding.

## Instructions

1. **Load `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `/commercial-legal:cold-start-interview` first -- I need to learn your playbook before I can review against it."

   Also read `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` → `## Review preferences` → `confirm_routing`. If the field is missing, treat it as `true`.
...
5. **Confirm routing if enabled.**
   ...
   If `confirm_routing` is `true` in `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` (or field is absent):
...
7. **Check for escalations:** If any issue exceeds the reviewer's authority per the `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` matrix, invoke **escalation-flagger** to route and draft the ask.
...
Add to `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` → `## Review preferences`:
...
Full review memo per the skill's format. Routing decision logged at the top. Deviation-by-deviation, specific redline language, named approver. Saved where `~/.hermes/legal-practice-profile/commercial-legal/CLAUDE.md` → House style says work product goes.
