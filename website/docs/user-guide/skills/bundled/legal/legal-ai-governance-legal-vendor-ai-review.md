---
title: "Ai Governance Legal Vendor Ai Review"
sidebar_label: "Ai Governance Legal Vendor Ai Review"
description: "Review vendor AI terms -- agreement, addendum, or ToS AI provisions -- against your governance positions; flag training-on-data, liability, model changes, an..."
---
...
| Path | `skills/legal/ai-governance-legal/vendor-ai-review` |
...
1. Read `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`. Confirm vendor governance positions are populated -- if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
...
Read `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance`. Also read `## AI policy commitments`
-- vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` contains `[PLACEHOLDER]`, surface this bounce:
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
If `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` doesn't define a position for a term on this list, ask: "Your playbook doesn't cover [term]. What's your default position, your acceptable fallback, and your automatic no? I'll add it to `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` so the next review is consistent."

---

## Playbook comparison

For each term above, compare what we found to the positions in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
...
> **Our position:** [from `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`]
> **Gap:** [specific delta -- or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate -- outside fallback"]

Use the severity ratings consistently (calibrated against `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md` positions):
...
- 🔴 **Critical** -- outside fallback; deployment should not proceed without resolution. Escalate per `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
...
Cross-check the vendor's terms against our AI policy commitments in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`, or "escalate -- outside fallback"
...
- It doesn't decide whether to accept terms outside the fallbacks. It routes those
  per the escalation table in `~/.hermes/legal-practice-profile/ai-governance-legal/CLAUDE.md`.
