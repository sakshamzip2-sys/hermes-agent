---
title: "Regulatory Legal Reg Feed Watcher"
sidebar_label: "Regulatory Legal Reg Feed Watcher"
...
| Path | `skills/legal/regulatory-legal/reg-feed-watcher` |
...
1. Load `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed config.
...
`~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run `/regulatory-legal:cold-start-interview --redo` to update, or edit `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md` directly.
...
- **Direct regulator RSS** -- fetch and parse any RSS URLs in ~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md feed
...
Each item gets a materiality tier per `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`:
...
- **Do** log the comment deadline to `~/.hermes/legal-practice-profile/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
...
- If comment tracking is enabled in ~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md: append to `~/.hermes/legal-practice-profile/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from ~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md
...
1. Look for `Digest output path` in `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
This skill reads the watchlist, materiality threshold, and feed configuration from `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output -- specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `/regulatory-legal:cold-start-interview --redo` or edit `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md`."
...
- Change the materiality threshold. If the filter is wrong, edit ~/.hermes/legal-practice-profile/regulatory-legal/CLAUDE.md.
