---
title: "Litigation Legal Privilege Log Review"
sidebar_label: "Litigation Legal Privilege Log Review"
...
| Path | `skills/legal/litigation-legal/privilege-log-review` |
...
1. Load `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` → review protocol, priv log format.
...
`~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md` → privilege log format, review protocol.

**Conflicts gate -- unbypassable.** Before reviewing a privilege log, check `~/.hermes/legal-practice-profile/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
**Before the privilege log is served on the opposing party (the consequential act -- this includes serving the log AND designating documents withheld or produced under a protective-order designation such as Confidential / Highly Confidential / AEO):** Read `## Who's using this` in `~/.hermes/legal-practice-profile/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
