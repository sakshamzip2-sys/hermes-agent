---
title: "Law Student Session — Run a focused N-question study session on a subject -- MBE, essay, or flashcards"
sidebar_label: "Law Student Session"
...
| Path | `skills/legal/law-student/session` |
...
2. Load `~/.hermes/legal-practice-profile/law-student/CLAUDE.md` → jurisdiction, exam format, weak subjects.
3. Load `~/.hermes/legal-practice-profile/law-student/study-plan.yaml` if it exists. Read `session_history` for this subject to weight subtopics toward where the student has been weak.
...
6. At session end, write session results:
   ...
   - If not: write to `~/.hermes/legal-practice-profile/law-student/session-history.yaml`.
7. Report:
