...
# Claude Code — OpenComputer Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the OpenComputer terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
OpenComputer interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
| `--effort <level>` | Reasoning depth: `low`, `medium`, `high`, `xhigh`, `max` (Claude Code 2.1.x rejects `auto` with a warning) | Both |
...
| `CLAUDE_CODE_EFFORT_LEVEL` | Default effort: `low`, `medium`, `high`, `xhigh`, or `max` |
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in OpenComputer terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
