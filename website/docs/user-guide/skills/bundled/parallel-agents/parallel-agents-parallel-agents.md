...
sidebar_label: "Parallel Agents"
description: "Run agents in parallel — dynamic workflows (hermes flow), background agent sessions (hermes agents), and agent teams (hermes team)"
---
...
Run agents in parallel — dynamic workflows (hermes flow), background agent sessions (hermes agents), and agent teams (hermes team). Use when a task benefits from many subagents, fan-out/verify pipelines, dispatching long-running work to the background, or a lead+teammates split with a shared task list.
