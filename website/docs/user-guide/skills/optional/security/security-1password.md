...
| | |
|---|---|
| Source | Optional — install with `opencomputer skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by OpenComputer |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during OpenComputer terminal calls (desktop app flow only)
...
### Service Account (recommended for OpenComputer)
...
## OpenComputer Execution Pattern (desktop app flow)

OpenComputer terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/opencomputer-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/opencomputer-op.sock"
