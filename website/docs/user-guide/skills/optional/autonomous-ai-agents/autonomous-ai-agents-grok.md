...
# Grok Build CLI — OpenComputer Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the OpenComputer terminal. Grok
...
1. Prepare stable input files first with OpenComputer tools (`read_file`,
...
2. **Don't conflate OpenComputer' xAI auth with the `grok` CLI's auth.** OpenComputer'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume OpenComputer' xAI auth carries
   over.
