---
title: "Opencomputer S6 Container Supervision"
sidebar_label: "Opencomputer S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the OpenComputer Docker image — adding new services, debugging profile gateways, understandin..."
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Opencomputer S6 Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the OpenComputer Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `oc skills install official/devops/opencomputer-s6-container-supervision` |
| Path | `optional-skills/devops/opencomputer-s6-container-supervision` |
| Version | `1.0.0` |
| Author | OpenComputer |
| License | MIT |
| Platforms | linux |
...
# OpenComputer s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the OpenComputer Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/opencomputer/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the OpenComputer and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-opencomputer-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
│       ├── chown /run/service (opencomputer-writable for runtime register)
...
│   ├── main-opencomputer/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `opencomputer dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid opencomputer opencomputer -p coder gateway run")
│   │   ├── down        (marker — present means "registered but don't auto-start")
│   │   └── log/run     (s6-log → $HERMES_HOME/logs/gateways/coder/current)
│   └── ...
│
└── CMD ("main program")               ← /opt/opencomputer/docker/main-wrapper.sh
    └── routes user args: bare exec | opencomputer subcommand | opencomputer (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/opencomputer/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-opencomputer-setup. |
...
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to opencomputer via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-opencomputer/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main opencomputer runs as the CMD, not as a supervised service. |
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `opencomputer gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main opencomputer to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/opencomputer/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to opencomputer via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main opencomputer is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to opencomputer via `s6-setuidgid opencomputer` at the top of run (unless you specifically need root).
...
`/command/` (where s6-overlay puts its binaries) is on PATH only for processes spawned by the supervision tree — services, cont-init.d, main-wrapper.sh. `docker exec <c> s6-svstat …` will fail with "command not found"; always use the absolute path `/command/s6-svstat`. The `opencomputer` binary works because the Dockerfile adds `/opt/opencomputer/.venv/bin` to the runtime `ENV PATH`.

### Profile directory ownership

The cont-init reconciler runs as opencomputer (`s6-setuidgid opencomputer` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> opencomputer profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$HERMES_HOME/profiles` to opencomputer on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user opencomputer` or rely on the stage2 chown sweep next reboot. Don't write files under `$HERMES_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `opencomputer -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `opencomputer profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `opencomputer-tool-quirks`: Specific OpenComputer-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with opencomputer built-in tools.
