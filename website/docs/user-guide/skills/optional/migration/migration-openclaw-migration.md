---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into OpenComputer"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into OpenComputer"
---
...
Migrate a user's OpenClaw customization footprint into OpenComputer. Imports OpenComputer-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> OpenComputer Migration

Use this skill when a user wants to move their OpenClaw setup into OpenComputer with minimal manual cleanup.
...
```bash
opencomputer claw migrate              # Full interactive migration
...
opencomputer claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `opencomputer setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the OpenComputer home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into OpenComputer memory entries
- merge OpenClaw command approval patterns into OpenComputer `command_allowlist`
- migrate OpenComputer-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to OpenComputer working-directory configuration
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen OpenComputer workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct OpenComputer destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of OpenComputer-compatible secrets, currently:
...
OpenComputer CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in OpenComputer. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a OpenComputer workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing OpenComputer skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of OpenComputer unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty OpenComputer targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean OpenComputer destination. Unsupported auth blobs must still be reported as skipped.
...
- OpenComputer persona state imported
- OpenComputer memory files populated with converted OpenClaw knowledge
