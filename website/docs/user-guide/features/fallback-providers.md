...
When a task's provider is set to `"auto"` (the default), OpenComputer first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, OpenComputer now honors user-configured fallback policy before using the built-in discovery chain:
