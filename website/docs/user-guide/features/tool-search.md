...
**Tool Search** is OpenComputer' opt-in progressive-disclosure layer for that
...
:::info Built-in OpenComputer tools never defer
The tools that make up OpenComputer' core capability set (`terminal`,
...
When the model invokes `tool_call`, OpenComputer **unwraps the bridge** and
...
- **No JS sandbox.** OpenComputer uses the simpler "structured tools" mode
