...
OpenComputer runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. OpenComputer
...
1. OpenComputer captures a baseline of current diagnostics for the file.
...
…). OpenComputer auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — OpenComputer installs both packages together when you
run `opencomputer lsp install typescript` or auto-install fires on first
use.

## CLI

```
opencomputer lsp status          # service state + per-server install status
opencomputer lsp list            # registry, optionally --installed-only
opencomputer lsp install <id>    # eagerly install one server
opencomputer lsp install-all     # try every server with a known recipe
opencomputer lsp restart         # tear down running clients
opencomputer lsp which <id>      # print resolved binary path
```

`opencomputer lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, OpenComputer installs binaries into
...
shared location — the staging dir is fully OpenComputer-owned and is
...
Servers are kept alive for the life of the OpenComputer process. There's
...
**`opencomputer lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`opencomputer lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `opencomputer lsp status`**
...
When `opencomputer lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `opencomputer lsp restart` to clear the set;
