...
**Multi-agent profiles**: When multiple OpenComputer instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
opencomputer memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `opencomputer` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — OpenComputer syncs them back at session init.
...
The `opencomputer honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `opencomputer memory setup honcho` (or run `opencomputer memory setup` and pick it from the list); the `opencomputer honcho` subcommand then appears on the next invocation.

```bash
opencomputer memory setup honcho    # Configure Honcho directly (works before activation)
opencomputer honcho status          # Connection status, config, and key settings
opencomputer honcho setup           # Redirects to `opencomputer memory setup` (post-activation alias)
opencomputer honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
opencomputer honcho peer            # Show or update peer names + dialectic reasoning level
opencomputer honcho mode            # Show or set recall mode (hybrid/context/tools)
opencomputer honcho tokens          # Show or set token budget for context and dialectic
opencomputer honcho identity        # Seed or show the AI peer's Honcho identity
opencomputer honcho sync            # Sync Honcho config to all existing profiles
opencomputer honcho peers           # Show peer identities across all profiles
opencomputer honcho sessions        # List known Honcho session mappings
opencomputer honcho map             # Map current directory to a Honcho session name
opencomputer honcho enable          # Enable Honcho for the active profile
opencomputer honcho disable         # Disable Honcho for the active profile
opencomputer honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `opencomputer honcho`

If you previously used the standalone `opencomputer honcho setup`:
...
No re-login or re-setup needed. Run `opencomputer memory setup` and select "honcho" — the wizard detects your existing config.
