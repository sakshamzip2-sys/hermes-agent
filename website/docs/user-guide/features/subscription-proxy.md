...
chat completions — use your OpenComputer-managed provider subscription as their
...
| Use case | "Use OpenComputer as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. OpenComputer stores
the refresh token in `~/.hermes/auth.json` — the same place all OpenComputer
...
If you see `not logged in`, run `opencomputer portal`. If you see
