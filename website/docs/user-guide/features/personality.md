...
description: "Customize OpenComputer's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

OpenComputer's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who OpenComputer is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

OpenComputer now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run OpenComputer with a custom home directory, it will use:
...
- OpenComputer creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- OpenComputer loads `SOUL.md` only from `HERMES_HOME`
- OpenComputer does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, OpenComputer falls back to a built-in default identity
...
If OpenComputer loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the OpenComputer instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change OpenComputer' default personality."
...
- how OpenComputer should handle uncertainty, disagreement, or ambiguity
...
## What OpenComputer injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, OpenComputer falls back to a built-in default identity ("You are OpenComputer, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
OpenComputer ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain OpenComputer, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives OpenComputer its persistent default personality unless the overlay meaningfully changes it.
...
- [SOUL.md Guide](/guides/use-soul-with-opencomputer)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how OpenComputer speaks
- `display.skin` and `/skin` affect how OpenComputer looks in the terminal
