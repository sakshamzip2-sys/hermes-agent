...
description: "Durable SQLite-backed task board for coordinating multiple OpenComputer profiles"
---
...
OpenComputer Kanban is a durable task board, shared across all your OpenComputer profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hermes/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.hermes/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `opencomputer kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `opencomputer kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/opencomputer-kanban-v1-spec.pdf` in the repository.
...
opencomputer kanban boards list

# Create a new board.
opencomputer kanban boards create atm10-server \
    --name "ATM10 Server" \
...
opencomputer kanban --board atm10-server list
opencomputer kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
opencomputer kanban boards switch atm10-server
opencomputer kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
opencomputer kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.hermes/kanban/current` — the slug persisted by `opencomputer kanban
   boards switch`.
4. `default`.
...
`opencomputer dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<opencomputer-home>/kanban/attachments/<task_id>/` for the default board, or
  `<opencomputer-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
...
opencomputer kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
opencomputer kanban watch

# 5. See the board (you)
opencomputer kanban list
opencomputer kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `opencomputer kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `opencomputer gateway
...
until one comes up — `opencomputer kanban create` warns about this at creation
time.

Running `opencomputer kanban daemon` as a separate process is **deprecated**;
...
opencomputer kanban create "nightly ops review" \
    --assignee ops \
...
```bash
opencomputer kanban complete t_abc t_def t_hij --result "batch wrap"
opencomputer kanban archive  t_abc t_def t_hij
opencomputer kanban unblock  t_abc t_def
opencomputer kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `opencomputer kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `opencomputer kanban` CLI.
...
**Zero schema footprint on normal sessions.** A regular `opencomputer chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```bash
opencomputer -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
opencomputer -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
opencomputer kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

opencomputer kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
opencomputer kanban create "Translate the docs site to French" \
...
```bash
opencomputer -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
opencomputer -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `opencomputer kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `opencomputer kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `opencomputer profile create --description "..."`, `opencomputer profile describe <name> --text "..."`, `opencomputer profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `opencomputer profile describe --auto`). |
...
```
opencomputer kanban init                                     # create kanban.db + print daemon hint
opencomputer kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
opencomputer kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
opencomputer kanban show <id> [--json]
opencomputer kanban assign <id> <profile>                    # or 'none' to unassign
opencomputer kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
opencomputer kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
opencomputer kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
opencomputer kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
opencomputer kanban diagnostics [--json]                     # board health snapshot (alias: diag)
opencomputer kanban link <parent_id> <child_id>
opencomputer kanban unlink <parent_id> <child_id>
opencomputer kanban claim <id> [--ttl SECONDS]
opencomputer kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
opencomputer kanban complete <id>... [--result "..."]
opencomputer kanban block <id> "<reason>" [--ids <id>...]
opencomputer kanban unblock <id>...
opencomputer kanban archive <id>...

opencomputer kanban tail <id>                                # follow a single task's event stream
opencomputer kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
opencomputer kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
opencomputer kanban runs <id> [--json]                       # attempt history (one row per run)
opencomputer kanban assignees [--json]                       # profiles on disk + per-assignee task counts
opencomputer kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
opencomputer kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `opencomputer gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
opencomputer kanban stats [--json]                           # per-status + per-assignee counts
opencomputer kanban log <id> [--tail BYTES]                  # worker log from ~/.hermes/kanban/logs/
opencomputer kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
opencomputer kanban notify-list [<id>] [--json]
opencomputer kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
opencomputer kanban context <id>                             # what a worker sees
opencomputer kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
opencomputer kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
opencomputer kanban create "nightly backup audit" \
...
```bash
opencomputer kanban swarm "Design a multi-region failover plan" \
...
Every `opencomputer kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `opencomputer chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `opencomputer kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `opencomputer kanban`. You don't have to leave the chat to drive the board.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`opencomputer kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `opencomputer kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/opencomputer-kanban-v1-spec.pdf`.
...
```bash
opencomputer kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
opencomputer kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
opencomputer kanban notify-list
opencomputer kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
opencomputer kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
opencomputer kanban runs t_abcd
...
**Bulk close caveat.** `opencomputer kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`opencomputer kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `opencomputer kanban heartbeat $TASK` to signal liveness during long operations. |
...
`opencomputer kanban tail <id>` shows these for a single task. `opencomputer kanban watch` streams them board-wide.
...
The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/opencomputer-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
