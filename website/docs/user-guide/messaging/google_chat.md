...
sidebar_position: 12
title: "Google Chat"
description: "Set up OpenComputer as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect OpenComputer to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your OpenComputer
...
> Run `opencomputer gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only OpenComputer can read (e.g.,
...
- Topic ID: `opencomputer-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `opencomputer-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a opencomputer restart)
...
- Principal: `opencomputer-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — OpenComputer calls
...
- **App name**: whatever you want users to see ("OpenComputer" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/opencomputer-chat-events`.
...
`ADDED_TO_SPACE` event that OpenComputer uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure OpenComputer
...
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/opencomputer-chat-events-sub
...
Install the dependencies the Google Chat adapter needs (no OpenComputer extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "OpenComputer is thinking…" marker, then
...
Thread support: when a user replies inside a thread, OpenComputer detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate OpenComputer session.
...
3. Download the JSON. Move it onto the host that runs OpenComputer.
4. Register the client with OpenComputer (run under the profile you want it scoped to):
...
opencomputer -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's OpenComputer home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, OpenComputer isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `opencomputer gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: OpenComputer will only attach the SA bearer
