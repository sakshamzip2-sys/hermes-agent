...
description: "Chat with OpenComputer from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with OpenComputer from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with OpenComputer](/guides/use-voice-mode-with-opencomputer).
...
```mermaid
flowchart TB
    subgraph Gateway["OpenComputer Gateway"]
...
For group chats, hooks, and automation flows, OpenComputer supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. OpenComputer keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; OpenComputer does not hide failures just because the text resembles a silence token.
...
```bash
opencomputer gateway setup        # Interactive setup for all messaging platforms
```
...
opencomputer gateway setup        # Configure messaging platforms interactively
opencomputer gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo opencomputer gateway install --system   # Linux only: install a boot-time system service
opencomputer gateway start        # Start the default service
opencomputer gateway stop         # Stop the default service
opencomputer gateway status       # Check default service status
opencomputer gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update OpenComputer to the latest version |
...
opencomputer pairing approve telegram XKGH5N7P

# Other pairing commands:
opencomputer pairing list          # View pending + approved users
opencomputer pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, OpenComputer appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
OpenComputer confirms immediately:
...
```bash
opencomputer gateway install               # Install as user service
opencomputer gateway start                 # Start the service
opencomputer gateway stop                  # Stop the service
...
sudo opencomputer gateway install --system
sudo opencomputer gateway start --system
sudo opencomputer gateway status --system
...
A system service needs root for every restart — including the automatic gateway restart at the end of `opencomputer update`. When `opencomputer update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart hermes-gateway` command (it never blocks on an interactive password prompt).
...
```bash
opencomputer gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `opencomputer update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo opencomputer update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/hermes-gateway`:

```
opencomputer ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed hermes-gateway*, /usr/bin/systemctl --no-ask-password start hermes-gateway*, /usr/bin/systemctl --no-ask-password restart hermes-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. OpenComputer will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple OpenComputer installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.hermes` uses `hermes-gateway`; other installations use `hermes-gateway-<hash>`. The `opencomputer gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
opencomputer gateway install               # Install as launchd agent
opencomputer gateway start                 # Start the service
opencomputer gateway stop                  # Stop the service
opencomputer gateway status                # Check status
...
- **HERMES_HOME** — scopes the gateway to your OpenComputer installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `opencomputer gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::
...
| Google Chat | `opencomputer-google_chat` | Full tools including terminal |
...
| DingTalk | `opencomputer-dingtalk` | Full tools including terminal |
| Feishu/Lark | `opencomputer-feishu` | Full tools including terminal |
| WeCom | `opencomputer-wecom` | Full tools including terminal |
| WeCom Callback | `opencomputer-wecom-callback` | Full tools including terminal |
| Weixin | `opencomputer-weixin` | Full tools including terminal |
| BlueBubbles | `opencomputer-bluebubbles` | Full tools including terminal |
| QQBot | `hermes-qqbot` | Full tools including terminal |
| Yuanbao | `hermes-yuanbao` | Full tools including terminal |
| Microsoft Teams | `hermes-teams` | Full tools including terminal |
| API Server | `opencomputer-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `opencomputer-webhook` | Full tools including terminal |
