...
Connect OpenComputer to **iMessage** through [Photon][photon], a managed
...
your first iMessage from OpenComputer — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, OpenComputer runs it in a
...
2. **Finds or creates** the `OpenComputer Agent` project on your account.
...
Photon uses the same authorization model as every other OpenComputer
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, OpenComputer replies with a pairing code. Approve it with:

```bash
opencomputer pairing approve photon <CODE>
```
...
By default OpenComputer responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `OpenComputer` and
`@OpenComputer agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and OpenComputer will reply.

## Status & troubleshooting

```bash
opencomputer photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line OpenComputer uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run opencomputer photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `opencomputer photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `opencomputer photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `opencomputer photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** OpenComputer sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | OpenComputer wake words  | JSON list / comma / newline regex patterns for group mentions |
