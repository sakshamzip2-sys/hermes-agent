...
description: Control your smart home with OpenComputer via Home Assistant integration.
...
OpenComputer integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "OpenComputer"
5. Copy the token
...
OpenComputer registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "OpenComputer".
...
from `config.yaml`. Double-check the file lives under the active OpenComputer profile
...
running OpenComputer — `curl -H "Authorization: Bearer <token>" <url>/api/` should
