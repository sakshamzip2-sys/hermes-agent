...
description: "Set up OpenComputer as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

OpenComputer can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The OpenComputer gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a OpenComputer limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. OpenComputer degrades gracefully if absent.
...
2. **Add** → name (e.g. `opencomputer-bot`) → role: **Admin**.
...
## Exposing OpenComputer to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the OpenComputer gateway has to be reachable from Meta's servers.  Three common ways:
...
3. Start the OpenComputer gateway: `opencomputer gateway`.
...
## Allowlist (OpenComputer-side)

In addition to Meta's recipient whitelist, OpenComputer has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.hermes/.env`:
...
The `opencomputer whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.
...
When the agent invokes any of these flows, OpenComputer uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
OpenComputer acknowledges inbound messages immediately:
...
OpenComputer TTS produces MP3. Two paths:
...
OpenComputer warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in OpenComputer. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  OpenComputer's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  OpenComputer doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Gateway not running** — check `opencomputer gateway` is up.
- **App Secret not set** — without it, OpenComputer refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in OpenComputer.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `opencomputer tools list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
```bash
opencomputer config set stt.provider openai
opencomputer config set stt.use_gateway true
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that OpenComputer will accept as authentic.
...
| | Baileys (`opencomputer whatsapp`) | Cloud API (`opencomputer whatsapp-cloud`) |
|---|---|---|
...
Most users running OpenComputer for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
