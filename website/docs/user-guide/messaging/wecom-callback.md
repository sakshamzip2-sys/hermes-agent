...
Connect OpenComputer to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
OpenComputer supports two WeCom integration modes:
...
> Run `opencomputer gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. OpenComputer decrypts the message, queues it for the agent
...
(Use `opencomputer gateway start` only after `opencomputer gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in OpenComputer and the token the
...
fixing, restart `opencomputer gateway run`.
...
Check `opencomputer gateway run` logs for the bound host/port. If the adapter bound to
