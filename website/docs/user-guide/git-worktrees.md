...
title: "Git Worktrees"
description: "Run multiple OpenComputers safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

OpenComputer is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with OpenComputer so each session has a clean, isolated working directory.

## Why Use Worktrees with OpenComputer?

OpenComputer treats the **current working directory** as the project root:

- CLI: the directory where you run `opencomputer` or `opencomputer chat`
...
git worktree add ../repo-feature feature/opencomputer-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/opencomputer-experiment` checked out in that directory

Now you can `cd` into the new worktree and run OpenComputer there:

```bash
cd ../repo-feature

# Start OpenComputer in the worktree
opencomputer
```
...
git worktree add ../repo-experiment-a feature/opencomputer-a
git worktree add ../repo-experiment-b feature/opencomputer-b
```
...
Each OpenComputer process:

- Works on its own branch (`feature/opencomputer-a` vs `feature/opencomputer-b`).
...
- OpenComputer checkpoint data under `~/.hermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per OpenComputer experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/opencomputer-checkpoints-docs`, `feature/opencomputer-refactor-tests`.
- **Commit frequently**
...
- **Avoid running OpenComputer from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `opencomputer -w` (Automatic Worktree Mode)

OpenComputer has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `opencomputer/opencomputer-<hash>`).
...
```bash
opencomputer -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `opencomputer -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each OpenComputer session its own clean checkout.
