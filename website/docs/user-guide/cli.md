...
sidebar_position: 1
title: "CLI Interface"
description: "Master the OpenComputer terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

OpenComputer's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `opencomputer setup --portal` — and you're ready to `opencomputer chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
OpenComputer also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `opencomputer --tui` — see the [TUI](tui.md) guide.
:::
...
opencomputer chat --model "anthropic/claude-sonnet-4"

# With a specific provider
opencomputer chat --provider nous        # Use Nous Portal
opencomputer chat --provider openrouter  # Force OpenRouter

# With specific toolsets
opencomputer chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
opencomputer -s hermes-agent-dev,github-auth
opencomputer chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
opencomputer --continue             # Resume the most recent CLI session (-c)
opencomputer --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
opencomputer chat --verbose
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the OpenComputer CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The OpenComputer CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `opencomputer --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`opencomputer -c` or `opencomputer --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend OpenComputer to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. OpenComputer supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for OpenComputer replies |
...
```bash
opencomputer -s hermes-agent-dev,github-auth
opencomputer chat -s github-pr-workflow -s github-auth
```

OpenComputer loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
personalities:
  ...
  pirate: "Arrr! Ye be talkin' to Captain OpenComputer..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches OpenComputer — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while OpenComputer is working, OpenComputer prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend OpenComputer to the background — just like any terminal process. The shell prints a confirmation:

```
OpenComputer has been suspended. Run `fg` to bring OpenComputer back.
```
...
Resume this session with:
  opencomputer --resume 20260225_143052_a1b2c3
...
```bash
opencomputer --continue                          # Resume the most recent CLI session
opencomputer -c                                  # Short form
opencomputer -c "my project"                     # Resume a named session (latest in lineage)
opencomputer --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
opencomputer --resume "refactoring auth"         # Resume by title
opencomputer -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `opencomputer sessions rename <id> <title>` from the command line. Use `opencomputer sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in OpenComputer's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
OpenComputer immediately confirms the task and gives you back the prompt:
