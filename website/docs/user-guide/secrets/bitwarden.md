...
2. OpenComputer stores that single token in `~/.hermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `opencomputer` (or the gateway, or a cron job) starts, after `~/.hermes/.env` has loaded, OpenComputer calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default OpenComputer **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every OpenComputer process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "OpenComputer keys").
...
4. **Machine accounts → New machine account → My OpenComputer machine** → **Projects** tab → grant Read access to your project.
...
```bash
opencomputer secrets bitwarden setup
```
...
```bash
opencomputer secrets bitwarden status
```

From now on, every `opencomputer` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `opencomputer secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `opencomputer secrets bitwarden status` | Show config + binary version + token presence |
| `opencomputer secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `opencomputer secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `opencomputer secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `opencomputer secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `opencomputer` invocations start fresh. |
...
Bitwarden never blocks OpenComputer startup. If anything goes wrong, you'll see a one-line warning in stderr and OpenComputer continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `opencomputer secrets bitwarden setup` |
...
- OpenComputer will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — OpenComputer does not auto-upgrade `bws` to "latest" because upstream release shapes can change.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple OpenComputer installations.
