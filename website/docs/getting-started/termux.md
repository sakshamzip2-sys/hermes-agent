...
description: "Run OpenComputer directly on an Android phone with Termux"
---

# OpenComputer on Android with Termux

This is the tested path for running OpenComputer directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop OpenComputer from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

OpenComputer now ships a Termux-aware installer path:
...
- links `opencomputer` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `opencomputer` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/opencomputer" "$PREFIX/bin/opencomputer"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `opencomputer` command persist across new shells without re-activating the venv every time.
...
### `opencomputer doctor` says ripgrep or Node is missing
