...
description: "Install and deploy OpenComputer with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

OpenComputer ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`opencomputer setup`, `opencomputer gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage opencomputer the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `opencomputer`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `opencomputer setup` walks you through provider selection, `opencomputer gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.hermes/`.
...
{ config, ... }: {
  services.hermes-agent = {
    ...
    environmentFiles = [ config.sops.secrets."opencomputer-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `opencomputer` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o opencomputer /dev/stdin /var/lib/opencomputer/env
```

```nix
services.hermes-agent.environmentFiles = [ "/var/lib/opencomputer/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `opencomputer` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `opencomputer` in your shell creates a separate `~/.hermes/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `opencomputer` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `opencomputer chat`, `opencomputer sessions list`, `opencomputer version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the opencomputer codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `opencomputer` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo opencomputer chat` manually.
:::
...
opencomputer config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, OpenComputer defaults to OpenRouter.
:::
...
```nix
services.hermes-agent.configFile = /etc/opencomputer/config.yaml;
```
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."opencomputer-env".path ]` |
...
| Override the opencomputer package | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/opencomputer"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). OpenComputer reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/opencomputer.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."opencomputer-env" = { format = "yaml"; };
  };

  services.hermes-agent.environmentFiles = [
    config.sops.secrets."opencomputer-env".path
  ];
}
```
...
```yaml
# secrets/opencomputer.yaml (encrypted with sops)
opencomputer-env: |
...
```nix
{
  age.secrets.hermes-env.file = ./secrets/opencomputer-env.age;
...
{
  services.hermes-agent = {
    authFile = config.sops.secrets."opencomputer/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). OpenComputer looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: OpenComputer loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. OpenComputer implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, OpenComputer prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u opencomputer` (native):

```bash
# Container mode
docker exec -it hermes-agent \
  opencomputer mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u opencomputer HERMES_HOME=/var/lib/opencomputer/.hermes \
  opencomputer mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
scp ~/.hermes/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/opencomputer/.hermes/mcp-tokens/
# Ensure: chown opencomputer:opencomputer, chmod 0600
```
...
When opencomputer runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `opencomputer setup` | Config is declarative — edit `settings` in your Nix config |
| `opencomputer config edit` | Config is generated from `settings` |
| `opencomputer config set <key> <value>` | Config is generated from `settings` |
| `opencomputer gateway install` | The systemd service is managed by NixOS |
| `opencomputer gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent opencomputer config set ...` is also blocked)
...
When container mode is enabled, opencomputer runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-hermes-agent-0.1.0  ──►  /nix/store/... (ro)
~/.hermes -> /var/lib/opencomputer/.hermes       (symlink bridge, per hostUsers)
/var/lib/opencomputer/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/opencomputer gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/opencomputer` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the opencomputer package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/opencomputer` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/opencomputer-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current opencomputer package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `opencomputer plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [opencomputer-lcm](https://github.com/stephenschoettler/opencomputer-lcm)):

```nix
services.hermes-agent.extraPlugins = [
  (pkgs.fetchFromGitHub {
    owner = "stephenschoettler";
    repo = "opencomputer-lcm";
    rev = "v0.7.0";
    hash = "sha256-...";
  })
];
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. OpenComputer discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."hermes_agent.plugins"]` (e.g., [rtk-opencomputer](https://github.com/ogallotti/rtk-opencomputer)):

```nix
services.hermes-agent.extraPythonPackages = [
  (pkgs.python312Packages.buildPythonPackage {
    pname = "rtk-opencomputer";
    version = "1.0.0";
    src = pkgs.fetchFromGitHub {
      owner = "ogallotti";
      repo = "rtk-opencomputer";
      rev = "v1.0.0";
      hash = "sha256-...";
    };
    format = "pyproject";
...
The package's `site-packages` is added to PYTHONPATH in the opencomputer wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core opencomputer dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `opencomputer` and `hermes-agent` binaries exist and `opencomputer version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `opencomputer --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true opencomputer config set ...` prints the NixOS error |
...
| `user` | `str` | `"opencomputer"` | System user |
| `group` | `str` | `"opencomputer"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/opencomputer"` | State directory (`HERMES_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `opencomputer` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `opencomputer gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the opencomputer user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.hermes` symlink to the service stateDir and are auto-added to the `opencomputer` group |
...
```
/var/lib/opencomputer/                     # stateDir (owned by opencomputer:opencomputer, 0750)
├── .hermes/                         # HERMES_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | OpenComputer binary + all Nix deps |
| `/data` | `/var/lib/opencomputer` | `rw` | All state, config, workspace |
| `/home/opencomputer` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
sudo rm /var/lib/opencomputer/.container-identity
...
```bash
# Native mode
sudo -u opencomputer cat /var/lib/opencomputer/.hermes/.env
...
| `opencomputer version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
| Permission denied on `/var/lib/opencomputer` | State dir is `0750 opencomputer:opencomputer` | Use `docker exec` or `sudo -u opencomputer` |
| `nix-collect-garbage` removed opencomputer | GC root missing | Restart the service (preStart recreates the GC root) |
...
| `unable to find user opencomputer` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
