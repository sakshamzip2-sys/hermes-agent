...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with OpenComputer — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working OpenComputer setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [OpenComputer Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="OpenComputer Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up OpenComputer for a team, bot, or always-on workflow
...
|---|---|---|
| I just want OpenComputer working on my machine | `opencomputer setup` | Run a real chat and verify it responds |
| I already know my provider | `opencomputer model` | Save the config, then start chatting |
| I want a bot or always-on setup | `opencomputer gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `opencomputer model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `opencomputer model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if OpenComputer cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install OpenComputer
### With the OpenComputer Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the OpenComputer Desktop installer](https://hermes-agent.nousresearch.com/desktop) from our website and run it.

### Without OpenComputer Desktop:
For a command-line only install without OpenComputer Desktop, run:
...
The single most important setup step. Use `opencomputer model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `opencomputer model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `opencomputer model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `opencomputer model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `opencomputer model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `opencomputer model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `opencomputer model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `opencomputer model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `opencomputer model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `opencomputer model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `opencomputer model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

OpenComputer separates secrets from normal config:
...
```bash
opencomputer config set model anthropic/claude-opus-4.6
opencomputer config set terminal.backend docker
opencomputer config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
opencomputer            # classic CLI
opencomputer --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
OpenComputer ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `opencomputer` vs `opencomputer --tui`.
:::
...
- OpenComputer replies without error
...
```bash
opencomputer --continue    # Resume the most recent session
opencomputer -c            # Short form
```
...
```bash
opencomputer gateway setup    # Interactive platform configuration
```
...
- `opencomputer tools` — tune tool access per platform
- `opencomputer skills` — browse and install reusable workflows
...
```bash
opencomputer config set terminal.backend docker    # Docker isolation
opencomputer config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the OpenComputer install directory (the curl installer placed it at
# ~/.hermes/hermes-agent on Linux/macOS or %LOCALAPPDATA%\opencomputer\hermes-agent on Windows):
...
Skills are on-demand instruction documents that teach OpenComputer how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

OpenComputer ships with a catalog of bundled skills already installed in `~/.hermes/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
opencomputer skills browse                      # list everything available
opencomputer skills search kubernetes           # find skills by keyword
opencomputer skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `opencomputer skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let OpenComputer ask what you need
```
...
|---|---|---|
| OpenComputer opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `opencomputer model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `opencomputer gateway setup` and check `opencomputer gateway status` |
| `opencomputer --continue` can't find old session | Switched profiles or session never saved | Check `opencomputer sessions list` and confirm you're in the right profile |
...
| `opencomputer doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |

## Recovery Toolkit

When something feels off, use this order:

1. `opencomputer doctor`
2. `opencomputer model`
3. `opencomputer setup`
4. `opencomputer sessions list`
5. `opencomputer --continue`
6. `opencomputer gateway status`
...
| Command | Description |
|---------|-------------|
| `opencomputer` | Start chatting |
| `opencomputer model` | Choose your LLM provider and model |
| `opencomputer tools` | Configure which tools are enabled per platform |
| `opencomputer setup` | Full setup wizard (configures everything at once) |
| `opencomputer doctor` | Diagnose issues |
| `opencomputer update` | Update to latest version |
| `opencomputer gateway` | Start the messaging gateway |
| `opencomputer --continue` | Resume last session |
