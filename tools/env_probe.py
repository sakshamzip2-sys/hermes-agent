...
machine as OpenComputer itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* OpenComputer ships under one Python (e.g. 3.11 in a bundled venv) while the
...
_REMOTE_BACKENDS = frozenset({
    "docker", "singularity", "modal", "daytona", "e2b", "ssh", "managed_modal",
})
