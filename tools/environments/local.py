...
# OpenComputer-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# OpenComputer-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a OpenComputer inference secret exactly
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local OpenComputer home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter OpenComputer-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. OpenComputer requires Git for Windows on Windows.\n"
...
def _resolve_hermes_bin_dir() -> str | None:
    ...
    oc install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
...
def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the oc install dir to ``existing_path`` if it's missing.
...
def _make_run_env(env: dict) -> dict:
    ...
    if path_key is not None:
        ...
        # Ensure the oc install dir is reachable so plugins can shell out
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, OpenComputer trusts them.
    """
