...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``oc tools`` repaint, tool-registration scans,
...
def hermes_xai_user_agent() -> str:
    """Return a stable OpenComputer-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"OpenComputer-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers OpenComputer-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.hermes/.env`` (the standard OpenComputer location) are honored —
