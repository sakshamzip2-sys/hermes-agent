"""
Lazy dependency installer for opt-in OpenComputer backends.

Many OpenComputer features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
remediation hint pointing at ``oc tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``oc update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``oc update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``oc update``. Never raises; lazy-install failures
