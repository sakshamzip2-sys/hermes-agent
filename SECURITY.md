# OpenComputer Security Policy

This document describes OpenComputer's trust model, names the one
...
security vulnerabilities. **OpenComputer does not operate a bug
bounty program.**
...
- Environment details (`opencomputer version`, commit SHA, OS, Python
  version).
...
OpenComputer is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running OpenComputer,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted OpenComputer access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in OpenComputer's documentation
...
OpenComputer supports two OS-level isolation postures. They address
...
OpenComputer supports this in two ways:

- **OpenComputer's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, OpenComputer's in-process heuristics
...
OpenComputer filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in OpenComputer
itself. Bugs in OpenComputer's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   OpenComputer does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, OpenComputer's own documentation, or reasonable
  operator expectations would predict — including cases where
  OpenComputer has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  opencomputer-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not OpenComputer's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in OpenComputer. Bugs in OpenComputer
...
- OpenComputer includes supply-chain guards for MCP server
