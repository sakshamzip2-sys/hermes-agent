# OpenComputer Has Had "Routines" Since March
...
OpenComputer equivalent — works today:
```bash
...
OpenComputer equivalent — works today:
```bash
opencomputer webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working OpenComputer implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | OpenComputer |
|---|---|---|
...
## Things OpenComputer Does That Routines Can't
...
```bash
opencomputer cron create "every 1h" \
...
OpenComputer has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
OpenComputer is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
opencomputer gateway setup    # enable webhooks
opencomputer webhook subscribe pr-review \
  --events "pull_request" \
...
*OpenComputer is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
