# OpenComputer Observer Hooks

OpenComputer observer hooks are the read-only telemetry contract for plugins that
...
They do not replace OpenComputer' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "opencomputer.observer.v1"
```

Hook callbacks are fail-open. OpenComputer catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so OpenComputer only builds
