...
When running OpenComputer inside Docker, the default `network_mode: host` gives the
...
The OpenComputer [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ hermes-agent │   │ opencomputer-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=opencomputer,opencomputer-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://opencomputer-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — OpenComputer trust model and vulnerability reporting
