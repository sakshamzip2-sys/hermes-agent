...
This means ``oc dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def list_diagnostics(
    ...
    AND for ``oc kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``oc kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``oc kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``oc kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current OpenComputer profile name for notify-sub ownership."""
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``oc profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``oc kanban decompose <task_id>``.
