...
Auto-tracks and cleans up ephemeral files created during OpenComputer
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HERMES_HOME` and `/tmp/opencomputer-*`.
...
- `is_safe_path()` rejects anything outside `HERMES_HOME` or `/tmp/opencomputer-*`
