...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated OpenComputer instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a OpenComputer profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active OpenComputer profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into OpenComputer system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active OpenComputer profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. OpenComputer session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — OpenComputer session_id ({timestamp}_{hex})
        ...
        # per-session: inherit OpenComputer session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'oc honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `oc honcho setup` to configure)."
            )
