...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/opencomputer>
...
```bash
opencomputer memory setup honcho   # configure Honcho directly (works on a fresh install)
opencomputer memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
opencomputer config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hermes/.env
```

> `opencomputer honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `opencomputer memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated OpenComputer instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active OpenComputer profile: `opencomputer` (default) or `hermes_<profile>`.
...
**Deployment shapes** (`opencomputer memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect OpenComputer to your own Telegram/Discord/etc.
...
- **`per-directory`** — basename of `$PWD`. Opening opencomputer in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — OpenComputer session ID (timestamp + hex). Every `opencomputer` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple OpenComputer profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`opencomputer`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active OpenComputer profile: `opencomputer` (default) or `hermes_<profile>` (e.g. `opencomputer -p coder` -> host key `hermes_coder`). Older `opencomputer.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `opencomputer memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `opencomputer honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `opencomputer memory setup`) |
| `opencomputer honcho status` | Show resolved config for active profile |
| `opencomputer honcho enable` / `disable` | Toggle Honcho for active profile |
| `opencomputer honcho mode <mode>` | Change recall or observation mode |
| `opencomputer honcho peer --user <name>` | Update user peer name |
| `opencomputer honcho peer --ai <name>` | Update AI peer name |
| `opencomputer honcho tokens --context <N>` | Set context token budget |
| `opencomputer honcho tokens --dialectic <N>` | Set dialectic max chars |
| `opencomputer honcho map <name>` | Map current directory to a session name |
| `opencomputer honcho sync` | Create host blocks for all OpenComputer profiles |
