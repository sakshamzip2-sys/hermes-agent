...
|-----|---------|-------------|
| `container_tag` | `opencomputer` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `opencomputer-{identity}` → `opencomputer-coder`). |
...
All Supermemory API calls send `x-sm-source: opencomputer`, and document writes stamp
`metadata.sm_source: opencomputer`. This is a **functional routing key, not telemetry**:
it groups OpenComputer-written memories into a dedicated "OpenComputer" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per OpenComputer profile:

```json
{
  "container_tag": "opencomputer-{identity}"
}
```

For a profile named `coder`, this resolves to `opencomputer-coder`. The default profile resolves to `opencomputer-default`. Without `{identity}`, all profiles share the same container.
