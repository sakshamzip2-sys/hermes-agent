...
Let the OpenComputer join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where opencomputer runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `opencomputer meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `opencomputer meet` CLI |
...
| `cli.py` | `opencomputer meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `opencomputer meet node {run,list,approve,remove,status,ping}` |
...
opencomputer meet install                                      # pip + Chromium
opencomputer meet setup                                        # preflight
opencomputer meet auth                                         # optional
opencomputer meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
opencomputer meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
opencomputer meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
opencomputer meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
opencomputer meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
opencomputer meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, opencomputer will **not** switch your system audio input automatically — the
...
opencomputer plugins enable google_meet
opencomputer meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
opencomputer meet node ping my-mac
