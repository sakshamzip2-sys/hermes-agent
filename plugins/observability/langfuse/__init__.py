"""langfuse — OpenComputer plugin for Langfuse observability.

Traces OpenComputer conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the OpenComputer plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``oc plugins enable
observability/langfuse`` or ``oc tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``oc tools`` or ~/.hermes/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the OpenComputer plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="OpenComputer turn",
                tags=["hermes", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="OpenComputer turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older OpenComputer branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current OpenComputer also has a turn-scoped
    ...
    # messages is a list only for legacy OpenComputer branches that fired
    # pre_llm_call with API messages directly. Current OpenComputer fires
