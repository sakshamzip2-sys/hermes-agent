...
Optional OpenComputer observability plugin that maps OpenComputer observer hooks to
...
not replace OpenComputer's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets OpenComputer emit NeMo Relay lifecycle events for
the work OpenComputer already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, OpenComputer can:

- Preserve OpenComputer execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
opencomputer plugins enable observability/nemo_relay
```
...
```bash
env HERMES_HOME=/tmp/opencomputer-nemo-relay-test \
  opencomputer plugins enable observability/nemo_relay
```
...
`HERMES_HOME` is the OpenComputer profile/config home used by both
`opencomputer plugins enable ...` and the later `opencomputer chat ...` run. If unset,
OpenComputer uses the user's default home, usually `~/.hermes`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HERMES_HOME=/tmp/opencomputer-nemo-relay-test
opencomputer plugins enable observability/nemo_relay
opencomputer chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `opencomputer` command you run is built from the
...
uv run opencomputer plugins enable observability/nemo_relay
uv run opencomputer chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so OpenComputer does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
OpenComputer keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, OpenComputer routes LLM and tool
...
For the full generic OpenComputer middleware contract, see
...
export HERMES_HOME=/tmp/opencomputer-nemo-relay-docs/opencomputer-home
mkdir -p "$HERMES_HOME"
...
This run starts a parent OpenComputer session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/opencomputer-nemo-relay-docs/subagent/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/opencomputer-nemo-relay-docs/subagent/atif
export HERMES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='OpenComputer E2E'
...
{"kind":"mark","name":"opencomputer.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "OpenComputer E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. OpenComputer dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/opencomputer-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/opencomputer-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/opencomputer-nemo-relay-docs/workdir/beta.txt
cd /tmp/opencomputer-nemo-relay-docs/workdir

export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/opencomputer-nemo-relay-docs/parallel/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/opencomputer-nemo-relay-docs/parallel/atif
...
- OpenComputer sessions map to `agent` scopes.
- OpenComputer API request hooks map to `llm` scope start/end events.
- OpenComputer tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses OpenComputer execution middleware to hand
...
Enable it for OpenComputer:

```bash
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/opencomputer-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, OpenComputer routes execution
through these boundaries:

```text
OpenComputer provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> OpenComputer provider adapter next_call(...)

OpenComputer tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> OpenComputer tool dispatcher next_call(...)
```
...
middleware for a local OpenComputer run. This path requires a NeMo Relay runtime that
...
```bash
export HERMES_HOME=/tmp/opencomputer-middleware-test/opencomputer-home
mkdir -p "$HERMES_HOME" /tmp/opencomputer-middleware-test/nemo-relay
...
cat > /tmp/opencomputer-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/opencomputer-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/opencomputer-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "OpenComputer Middleware E2E"
agent_version = "local"
...
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/opencomputer-middleware-test/nemo-relay/plugins.toml
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "OpenComputer Middleware E2E",
    "version": "local",
