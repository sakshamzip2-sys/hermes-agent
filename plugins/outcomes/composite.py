"""Composite per-turn score — pure-arithmetic fusion of implicit signals.

Ported from OpenComputer v1 (``evolution/composite_scorer.py``). Combines implicit
per-turn signals into a single composite_score in [0, 1] with NO LLM call. Designed
to give a useful score even when the user is silent (baseline 0.5 anchors the
absent-signal case).

Weights are tuned to prevent reward-hacking:
- tool_success_rate alone is capped at 0.20 (the agent can't game by avoiding tools);
- a correction always penalises more than an affirmation rewards (anti-sycophancy);
- abandonment is only a modest negative so trailing "anything else?" prompts don't pay off.
"""
...
def compute_composite_score(
    *,
    ...
    Baseline 0.5 ensures silent turns aren't crashed to zero. Tool success is the
    largest positive signal; correction is the largest negative signal (with
    self-cancel close behind).
    """
