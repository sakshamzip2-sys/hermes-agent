"""
IRC Platform Adapter for OpenComputer.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the OpenComputer.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :OpenComputer")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :OpenComputer shutting down")
...
def interactive_setup() -> None:
    """Interactive `oc gateway setup` flow for the IRC platform.
    ...
    print_info("Connect OpenComputer to an IRC network. Uses Python stdlib — no extra packages needed.")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :OpenComputer (cron)")
