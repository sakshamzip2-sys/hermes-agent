...
TypeScript-only, OpenComputer runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-OpenComputer-Sidecar-Token`.
...
There is no separate `login` command; like every other OpenComputer channel,
...
Run `opencomputer photon status` to see what's configured.
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `opencomputer photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `opencomputer photon status`, a DM and a group roundtrip,
