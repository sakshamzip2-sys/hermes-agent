...
Small Node helper that bridges OpenComputer to Photon's Spectrum SDK
(`spectrum-ts`).  OpenComputer is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-OpenComputer-Sidecar-Token`)
...
The OpenComputer plugin's `opencomputer photon setup` command runs `npm install`
here automatically.
