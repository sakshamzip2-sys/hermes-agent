"""SimpleX Chat platform adapter (OpenComputer plugin).
...
This adapter ships as a OpenComputer platform plugin under
``plugins/platforms/simplex/``. The OpenComputer plugin loader scans the
...
discoverable and ``oc setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside OpenComputer — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``oc setup gateway`` → SimpleX.
