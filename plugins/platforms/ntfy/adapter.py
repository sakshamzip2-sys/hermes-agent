"""ntfy platform adapter (OpenComputer plugin).
...
a OpenComputer dependency.

This adapter ships as a OpenComputer platform plugin under
``plugins/platforms/ntfy/``. The OpenComputer plugin loader scans the
...
async def _standalone_send(
    ...
    *,
    ...
    ``oc cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a OpenComputer dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `oc gateway status` without
