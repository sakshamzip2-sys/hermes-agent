...
There are two ways to add a platform to the OpenComputer gateway:
...
**zero changes to core OpenComputer code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `opencomputer gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the OpenComputer core.
...
```python
"hermes-gateway": {
    "includes": [..., "opencomputer-your-platform"]
}
```
