...
# Audio file extensions OpenComputer recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through OpenComputer' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. OpenComputer-created Telegram private-chat
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; OpenComputer/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active OpenComputer profile and shared OpenComputer root both contain control
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and OpenComputer
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    OpenComputer-managed cache, under an operator-allowlisted root
...
class BasePlatformAdapter(ABC):
    ...
    # OpenComputer commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: OpenComputer sends
