...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. OpenComputer upgrades overwrite this
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          ...
          detached ``oc update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``oc update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `oc setup gateway` / `oc gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``oc update --gateway`` watcher. Button
