...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach OpenComputer commands.
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running OpenComputer version."""
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — OpenComputer default runtime
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        ``oc skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from hermes_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `oc skills diff <name>` is a
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `oc bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``oc debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update OpenComputer to the latest version.

        Spawns ``oc update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``oc update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update OpenComputer')}"
        ...
        # Spawn `oc update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `oc update --gateway`
