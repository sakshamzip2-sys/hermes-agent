...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    joined = " ".join(tokens)
    # ``oc`` is the OpenComputer-rebranded console entrypoint (the live gateway
    # runs as ``.../oc gateway run``); recognize it alongside ``hermes`` so the
    # rebranded command isn't invisible to gateway lifecycle checks.
    has_gateway_entry = (
        "hermes_cli.main" in joined
        or "hermes_cli/main.py" in joined
        or any(
            t.rsplit("/", 1)[-1] in ("hermes", "hermes.exe", "oc", "oc.exe")
            for t in tokens
        )
    )
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the OpenComputer gateway."""
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``oc gateway stop`` on Windows would be misclassified as an
