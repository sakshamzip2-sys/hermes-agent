"""Shared constants for OpenComputer Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local OpenComputer home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local OpenComputer home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local OpenComputer home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default OpenComputer home path."""
...
def get_hermes_home() -> Path:
    """Return the OpenComputer home directory (default: platform-native path).
...
def get_default_hermes_root() -> Path:
    """Return the root OpenComputer directory for profile-level operations.

    In standard deployments this is the platform-native OpenComputer home
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when OpenComputer is installed
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a OpenComputer subdirectory with backward compatibility.
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding OpenComputer profile HOME.

    ``HERMES_HOME`` scopes OpenComputer state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply OpenComputer' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def parse_reasoning_effort(effort: str) -> dict | None:
    ...
    These map 1:1 to Anthropic's ``output_config.effort`` ladder
    (low < medium < high < xhigh < max); the OpenComputer router accepts the
    same set as the OpenAI-style ``reasoning_effort``. "max" is honored on
    Opus 4.6+/Sonnet 4.6 and ignored where unsupported.
