"""ACP auth helpers — detect and advertise OpenComputer authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active OpenComputer runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if OpenComputer can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for OpenComputer.
    ...
    not have OpenComputer provider credentials configured yet, so OpenComputer always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate OpenComputer using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure OpenComputer provider",
            description=(
                "Open OpenComputer' interactive model/provider setup in a terminal. "
                "Use this when OpenComputer has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
