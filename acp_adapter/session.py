"""ACP session manager — maps ACP sessions to OpenComputer AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when OpenComputer itself is running in WSL.

    Windows ACP clients can launch ``oc acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch OpenComputer from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed OpenComputer."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by OpenComputer AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current OpenComputer runtime provider configuration.
