...
TIPS = [
    ...
    "Ctrl+Z suspends OpenComputer to the background — run fg in your shell to resume.",
    ...
    "oc chat -s github-pr-workflow preloads a skill at launch.",
    "oc chat -q \"query\" runs a single non-interactive query and exits.",
    ...
    # --- CLI Subcommands ---
    "oc doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "oc dump outputs a compact setup summary — great for bug reports.",
    "oc config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "oc config edit opens config.yaml in your default editor.",
    "oc config check scans for missing or stale configuration options.",
    "oc sessions browse opens an interactive session picker with search.",
    "oc sessions stats shows session counts by platform and database size.",
    "oc sessions prune --older-than 30 cleans up old sessions.",
    "oc skills search react --source skills-sh searches the skills.sh public directory.",
    "oc skills check scans installed hub skills for upstream updates.",
    "oc skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "oc skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "oc mcp add github --command npx adds MCP servers from the command line.",
    "oc mcp serve runs OpenComputer itself as an MCP server for other agents.",
    "oc auth add lets you add multiple API keys for credential pool rotation.",
    "oc completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "oc logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "oc backup creates a zip backup of your entire OpenComputer home directory.",
    "oc profile create coder creates an isolated profile that becomes its own command.",
    "oc profile create work --clone copies your current config and keys to a new profile.",
    "oc update syncs new bundled skills to ALL profiles automatically.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call OpenComputer tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'oc profile create coder' creates the 'coder' command.",
    "oc profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, OpenComputer prints a resume command with session ID and stats.",
    "oc sessions export backup.jsonl exports all sessions for backup or analysis.",
    "oc -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "oc skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Gateway & Messaging ---
    "OpenComputer runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "oc gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to OpenComputer via a local macOS server.",
    ...
    "OpenComputer loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "oc mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "oc mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    ...
    "oc chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "oc plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "oc acp runs OpenComputer as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make OpenComputer your own.",
    ...
    "Before a gateway session reset, OpenComputer auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope OpenComputer state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'oc profile create ls' would be rejected.",
    "oc profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "oc claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — OpenComputer auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of OpenComputer config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while OpenComputer is working.',
    ...
    # --- Curator ---
    'oc curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "oc curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'oc curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'oc auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'oc dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'oc dashboard embeds the full OpenComputer TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'oc -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes OpenComputer block commands when the tirith scanner itself errors out.',
