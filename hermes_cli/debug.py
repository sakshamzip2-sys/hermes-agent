"""``oc debug`` debug tools for OpenComputer.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``oc debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``oc debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, OpenComputer version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `oc debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``oc debug share`` invocation added ~20 MB of resident Python
    ...
    ``oc debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``oc dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``oc dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``oc debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "oc debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `oc debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"\nShare these links with the OpenComputer team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("  Deletes paste.rs pastes uploaded by 'oc debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``oc debug`` call.
    ...
    # best-effort — any failure is swallowed so ``oc debug`` stays
