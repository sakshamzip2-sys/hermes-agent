"""Per-provider model-selection wizard flows for ``oc setup`` / ``oc model``.
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `oc setup` from scratch. OpenRouter
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `oc auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``oc model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``oc model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``oc auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"OpenComputer will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `oc model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  OpenComputer will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  OpenComputer will install it now (the preflight below "
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates OpenComputer turns to `copilot --acp`.")
    print("  OpenComputer currently starts its own ACP subprocess for each request.")
    print("  OpenComputer uses your selected model as a hint for the Copilot ACP session.")
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   OpenComputer typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with OpenComputer, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'oc model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
