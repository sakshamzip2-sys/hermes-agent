"""
OpenComputer Agent — Web UI server.
...
try:
    ...
    # running `oc dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``oc gateway run`` — but the
    desktop app spawns a ``oc dashboard`` backend, not a gateway, so a cron
...
app = FastAPI(title="OpenComputer Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-OpenComputer-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-OpenComputer-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates OpenComputer (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``oc model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real OpenComputer provider slug — fine.
    ...
    1. Vendor-name → OpenComputer-provider mapping: when the provider string is
       not a known OpenComputer provider/alias (e.g. ``moonshotai``, ``x-ai`` is
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``oc update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's OpenComputer root is actually /opt/data
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``oc status`` and the
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``oc <subcommand>`` detached and record the Popen handle.
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["oc", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``oc gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``oc gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``oc gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``oc update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "OpenComputer updates are managed outside this dashboard in "
    ...
    try:
        ...
        _log.exception("Failed to spawn oc update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a OpenComputer update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``oc update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed OpenComputer version string
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another OpenComputer surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    OpenComputer supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `oc model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `oc model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``oc model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``oc model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `oc model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `oc model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run OpenComputer from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect OpenComputer to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use OpenComputer from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect OpenComputer to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use OpenComputer in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use OpenComputer through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from OpenComputer via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to OpenComputer through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect OpenComputer to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use OpenComputer inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use OpenComputer through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect OpenComputer to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect OpenComputer to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose OpenComputer as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in oc gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in oc gateway setup",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "OpenComputer Agent").strip() or "OpenComputer Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, OpenComputer appears
...
# the canonical ``oc auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — OpenComputer-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"OpenComputer PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into OpenComputer even
    when they also have a separate OpenComputer-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "oc auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "oc auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "oc auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "oc auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "oc auth add anthropic",
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``oc model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `oc model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"oc auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `oc model` picker; _OAUTH_OVERRIDES supplies per-provider
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both OpenComputer file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``oc auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `oc auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `oc auth add xai-oauth`'s credential-pool insert.
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `oc sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `oc sessions prune`)."""
...
# servers managed here show up under `oc mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `oc mcp catalog` / `oc mcp install` read.  ``profile`` scopes
...
# spawn the real `oc gateway <verb>` so behaviour matches the CLI exactly.
...
# `oc memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `oc memory setup` to configure a new one.",
            )
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `oc import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`oc -p <name> skills install`)
...
def _profile_setup_command(name: str) -> str:
    ...
    return "oc setup" if name == "default" else f"{name} setup"
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``oc profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``oc profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``oc profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``oc tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``oc tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``oc tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``oc tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``oc tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``oc -p <profile> tools post-setup``.
...
#
# The endpoint spawns the same ``oc --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``oc --tui`` would run.  Tests monkeypatch this
    ...
    — the same propagation ``oc -p <name>`` performs. The in-process
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall OpenComputer inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the OpenComputer backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "OpenComputer Teal",         "description": "Classic dark teal — the canonical OpenComputer look"},
    {"name": "default-large", "label": "OpenComputer Teal (Large)", "description": "OpenComputer Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with OpenComputer Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `oc dashboard register` (Nous Portal) or "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  OpenComputer Web UI → http://{host}:{actual_port}")
