...
def detect_service_manager() -> ServiceManagerKind:
    ...
    # `oc gateway start/stop/restart` fell through to host code that spawns
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        if profile == "default":
            gateway_cmd = "oc gateway run --replace"
        else:
            gateway_cmd = f"oc -p {shlex.quote(profile)} gateway run --replace"
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``oc -p <profile> gateway start``.
