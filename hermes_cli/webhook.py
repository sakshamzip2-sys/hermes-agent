"""oc webhook — manage dynamic webhook subscriptions from the CLI.
...
def _setup_hint() -> str:
  ...
  Then start the gateway: oc gateway run
"""
...
def webhook_command(args):
    """Entry point for 'oc webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: hermes webhook {subscribe|list|remove|test}")
        print("Run 'oc webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (oc gateway run).\n")
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (oc gateway run)")
