...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `oc model`,
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `oc model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `oc model`, then append the selection to the chain."""
    ...
    print("  `oc model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `oc fallback list` to view, or `oc fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``oc fallback [subcommand]``."""
