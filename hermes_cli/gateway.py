...
Handles: oc gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `oc update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "oc CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``oc gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``oc gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``oc update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            OpenComputer profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to OpenComputer profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``oc update`` users
    # with no running gateway until they re-invoke ``oc gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}oc gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}oc gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `oc gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``oc gateway status`` output so users with
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our OpenComputer Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``oc gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "OpenComputer Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional OpenComputer root to compare against. Used when
...
# Legacy service names from older OpenComputer installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy OpenComputer gateway units.

    Detects unit files installed by older OpenComputer versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy OpenComputer gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy OpenComputer gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy OpenComputer gateway unit(s) detected from an older install:")
    ...
    print_info("    oc gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy OpenComputer gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy OpenComputer gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: oc gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo oc gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    oc gateway uninstall")
    print_info("    sudo oc gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so OpenComputer can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo oc gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo oc gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo oc gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode OpenComputer often sets ``HOME`` to a profile-scoped directory, but
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``oc update`` later relocates/removes — the path
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``oc gateway restart``
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current OpenComputer install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo oc gateway uninstall --system")
    print_info("         oc gateway install")
    print_info("         oc gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}oc gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}oc gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}oc gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `oc gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `oc gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}oc gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}oc gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}oc gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}oc gateway start{scope_flag}"
        )
...
# process invocation (each ``oc gateway start/stop/status`` call).
...
# (the documented `nohup oc gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup oc gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: oc gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup oc gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current OpenComputer install"
    )
    return True
...
def launchd_stop():
    ...
    # `oc gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current OpenComputer install")
    else:
        print("⚠ Service definition is stale relative to the current OpenComputer install")
        print("  Run: oc gateway start")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``oc gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``oc gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``oc gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'oc gateway restart' to replace it,")
    print("  or 'oc gateway stop' first.")
    print("  Or use 'oc gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the OpenComputer gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the OpenComputer command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `oc gateway run` still
    ...
    # /restart) rather than through `oc gateway restart` which already
    ...
    # `oc gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ OpenComputer Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where OpenComputer delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. OpenComputer connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and OpenComputer will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``oc setup gateway`` without needing the gateway to be running.
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'oc pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: oc pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'oc gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. OpenComputer will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. OpenComputer will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `oc gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `oc pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `oc pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "OpenComputer"')
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: oc gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo oc gateway install --system"
                        )
                    print_info("  Or run in foreground:  oc gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: oc gateway run")
                print_info(
                    "  For persistence:   tmux new -s hermes 'oc gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: oc gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup oc gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'oc gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``oc gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``oc gateway install|uninstall|start|stop|restart``
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Consider running in foreground instead: oc gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'oc gateway run'"
                )
                print()
            ...
            print()
            print(
                "  oc gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hermes 'oc gateway run'         # persistent via tmux"
            )
            print(
                "  nohup oc gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  oc profile create <name>     # creates the s6 service slot")
                print("  oc -p <name> gateway start   # bring it up via s6")
                print("  oc status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: oc gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: oc gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: oc gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  oc profile delete <name>     # tears down the s6 service slot")
                print("  oc -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `oc -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: oc gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `oc gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `oc gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: oc gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  oc gateway install")
                    print("  sudo oc gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  oc gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup oc gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'oc gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  oc gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  oc gateway install  # Install as user service")
                    print(
                        "  sudo oc gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy OpenComputer gateway unit files from
