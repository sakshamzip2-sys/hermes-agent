...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other OpenComputer loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # separate `oc -p <profile> chat -q` subprocesses:
...
class PluginManifest:
    ...
    # lookups and by ``oc plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``oc honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``oc <subcommand>``
    ...
    def register_slack_action_handler(
        ...
        OpenComputer' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``oc model → Configure auxiliary models`` picker
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # for the same reason: every platform OpenComputer ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `oc plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `oc plugins list`.
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``oc tools`` TUI so plugin-provided toolsets appear
