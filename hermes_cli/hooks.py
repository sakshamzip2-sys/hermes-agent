"""oc hooks — inspect and manage shell-script hooks.
...
def hooks_command(args) -> None:
    """Entry point for ``oc hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hermes hooks {list|test|revoke|doctor}")
        print("Run 'oc hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `oc hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `oc hooks test` and `oc hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (OpenComputer wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `oc hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `oc hooks doctor` would execute
