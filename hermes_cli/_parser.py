...
Examples:
    ...
    oc dashboard              Start web UI dashboard (port 9119)
    oc dashboard --stop       Stop running dashboard processes
    oc dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from OpenComputer itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
