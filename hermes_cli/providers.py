"""
Single source of truth for provider identity in OpenComputer.
...
2. **OpenComputer overlays** — transport type, auth patterns, aggregator flags,
...
# -- OpenComputer overlay ----------------------------------------------------------
# OpenComputer-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """OpenComputer-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. OpenComputer overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + OpenComputer overlay
    ...
    if overlay is not None:
        # OpenComputer-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
