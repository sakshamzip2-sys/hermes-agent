...
Pure logic: walks a OpenComputer config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `oc doctor` and a future `oc migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a OpenComputer config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a OpenComputer config and return retirement issues.
