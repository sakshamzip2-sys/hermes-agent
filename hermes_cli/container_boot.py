...
def reconcile_profile_gateways(
    *,
    ...
    is what ``oc gateway start`` (no ``-p``) targets. Without it,
    bare ``oc gateway start`` inside the container would land on
    ...
    # ``oc gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `oc profile create` (config.yaml
            # is not — that comes later via `oc setup`). Use it as the
            ...
            # via ``oc -p default-named gateway start`` if they
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``oc dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `oc -p <profile> gateway start`
