...
Usage::
    ...
    oc profile create coder          # fresh profile + bundled skills
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            oc_exe = shutil.which("oc") or shutil.which("hermes") or "oc"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(oc_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running OpenComputer. If we
    ...
    try:
        ...
        # not be migrated. The next `oc doctor --fix` can still surface the
...
def create_profile(
    ...
    # Cloned configs can be older than the running OpenComputer (or predate schema
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: oc uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: oc profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: oc profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: oc profile import <archive> --name <name>"
        )
