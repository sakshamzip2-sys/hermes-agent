...
on Japanese locales, etc.).  OpenComputer's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. OpenComputer is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the OpenComputer-managed Git install directories so
    ...
    # install without OpenComputer-managed PortableGit).
    ...
    # stdin is re-configured for completeness; OpenComputer's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and OpenComputer's
    ``oc config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching OpenComputer (User env var in Windows Settings, or
    export in a PowerShell profile) and OpenComputer picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known OpenComputer-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess OpenComputer spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known OpenComputer-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # OpenComputer venv Scripts directory — host of the hermes.exe shim itself,
