"""CLI subcommand: `oc curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `oc curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`oc curator status` and run `oc curator run` (no flag) to apply."
            )
        else:
            print(
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`oc curator backup` or wait for the next curator run."
            )
        else:
            print(
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="oc curator")
    register_cli(parser)
