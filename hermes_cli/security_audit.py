"""On-demand supply-chain audit for OpenComputer installs.

Scans three surfaces a OpenComputer user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The OpenComputer venv (every PyPI dist via ``importlib.metadata``).
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `oc security audit`."""
