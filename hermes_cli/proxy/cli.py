"""CLI handlers for the ``oc proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "oc proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"oc auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting OpenComputer proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("OpenComputer proxy upstream adapters\n")
...
def cmd_proxy(args: Any) -> int:
    ...
    print(
        "oc proxy — local OpenAI-compatible proxy that attaches your\n"
