...
``<root>`` is the **shared OpenComputer root** (the parent of any active
...
``oc -p <profile>`` joins the same board as the dispatcher that
...
the profile's OpenComputer home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``oc kanban boards
...
def kanban_home() -> Path:
    """Return the shared OpenComputer root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``oc kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``oc kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``oc kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``oc kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``oc gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources OpenComputer treats as user-controlled-but-trusted on the
...
def create_task(
    ...
    *,
    ...
    each name to ``oc --skills ...``. Use this to pin a task to a
...
# Canonical sort-order mappings for ``oc kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `oc kanban recompute` (issue #22459).
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``oc kanban block <id>``).  This is a deliberate handoff that
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold OpenComputer' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside OpenComputer-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``oc kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a OpenComputer
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real OpenComputer profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real OpenComputer profile.
        # `_default_spawn` invokes ``oc -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a OpenComputer
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `oc kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound OpenComputer CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved OpenComputer shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved OpenComputer executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       OpenComputer is launched from a venv and the ``hermes`` shim is not on
...
def _default_spawn(
    ...
    *,
    ...
    # env, and when the child process starts `oc -p <name>` the
    ...
    # `oc kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install OpenComputer or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default OpenComputer root exists
