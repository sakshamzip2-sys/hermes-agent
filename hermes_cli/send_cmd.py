"""CLI subcommand: ``oc send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"oc send: {file_path} is not a text file. --file reads the "
            ...
            print(f"oc send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"oc send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"oc send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"oc send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"oc send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `oc gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``oc send`` must replicate when invoked standalone:
    ...
    intentionally reimplement the minimum needed here so ``oc send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "oc send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
    ...
    if message is None or not message.strip():
        print(
            "oc send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `oc send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform OpenComputer "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `oc send --list telegram`.",
    )
