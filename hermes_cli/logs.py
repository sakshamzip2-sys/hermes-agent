"""``oc logs`` — view and filter OpenComputer log files.
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when OpenComputer runs — try 'oc chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'oc chat' to generate logs)")
