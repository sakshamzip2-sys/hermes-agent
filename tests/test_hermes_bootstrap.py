...
The bootstrap module is imported at the top of every OpenComputer entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every OpenComputer entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the OpenComputer CLI
...
class TestEntryPointsImportBootstrap:
    """Every OpenComputer entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke OpenComputer as a process.  Each one must
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``oc update`` (git-reset landed new code but
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # oc start even when hermes_bootstrap hasn't been
