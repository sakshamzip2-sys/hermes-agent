"""Tests for the post-pull syntax guard in ``oc update``.
...
markers in ``hermes_cli/config.py``; users who ran ``oc update`` in
...
# release; if a future ``oc update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"oc update would brick users. Error: {error}"
    )
