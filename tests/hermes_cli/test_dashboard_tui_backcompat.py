"""Regression test: `oc dashboard --tui` must not hard-crash.

Older OpenComputer desktop app shells (<= 0.15.x) spawn the backend as::

    oc dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "OpenComputer couldn't
