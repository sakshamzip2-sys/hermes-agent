"""Tests for the multi-board kanban layer (``oc kanban boards …``).
...
* CLI surface: ``oc kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``oc kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
