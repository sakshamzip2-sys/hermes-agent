...
only when they still resolve into the OpenComputer-managed node dir.
"""
...
def test_removes_dangling_symlink_into_hermes_node(fake_home):
    """A link into the OpenComputer node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the OpenComputer links that exist; ignores the ones that don't."""
    hermes_home = fake_home / ".hermes"
    node_bin = _make_hermes_node(hermes_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are OpenComputer-managed; node is a real user binary.
