...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``oc gateway run`` (no ``-p``) to mean "the root HERMES_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        gateway is launched as bare ``oc gateway run`` with
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `oc gateway run` still honors
