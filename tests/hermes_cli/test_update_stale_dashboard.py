"""Tests for the stale-dashboard handling run at the end of ``oc update``.

``oc update`` detects ``oc dashboard`` processes left over from the
...
class TestFindStaleDashboardPids:
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12346, "oc dashboard --port 9120 --no-open"),
    ...
    def test_exclude_pids_filters_specified_pids(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "oc dashboard --port 9119"),
                    _ps_line(22222, "oc dashboard --port 9120"),
                    _ps_line(33333, "oc dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "oc dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestWindowsWmicEncoding:
    ...
    `oc update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
