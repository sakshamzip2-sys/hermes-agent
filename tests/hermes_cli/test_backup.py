...
class TestShouldExclude:
    ...
    def test_includes_nested_hermes_agent_in_skills(self):
        """skills/autonomous-ai-agents/opencomputer-agent/ must NOT be excluded —
        only the root-level hermes-agent/ repo is skipped."""
        from hermes_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/opencomputer-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/opencomputer-agent/sub/item.txt"))
...
class TestBackup:
    ...
    def test_includes_nested_hermes_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        (nested / "SKILL.md").write_text("# OpenComputer Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            ...
            assert "skills/autonomous-ai-agents/opencomputer-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/opencomputer-agent/sub/item.txt" in names
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid OpenComputer backup."""
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "oc -p coder" in coder_wrapper
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``oc update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``oc backup`` would, and should exclude the same directories."""
        from hermes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
...
class TestPreMigrationBackup:
    ...
    ``oc claw migrate`` runs before mutating ~/.hermes/."""
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``oc backup`` / ``create_pre_update_backup`` — no drift."""
        from hermes_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid OpenComputer
        backup — `oc import` should accept it."""
...
class TestRestoreCronJobsIfEmptied:
    """`oc update` config migration can leave cron/jobs.json valid-but-empty,
