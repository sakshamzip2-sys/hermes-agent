...
Before the fix, ``oc whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``oc gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``oc whatsapp`` with an existing paired session and
