"""Regression test: ``oc mcp add --command`` must not clobber the
...
result, ``oc mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`oc mcp add foo --url ...` must keep args.command == "mcp".
