...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `oc tools` → X (Twitter) Search to flip the
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `oc tools`,
    ...
    # User explicitly opted into spotify but not x_search via `oc tools`.
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # auto-recovered by _get_platform_tools so saving via `oc tools`
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    ...
    ``oc tools``, but hermes-cli stays in the config and re-enables
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `oc tools` must reach the
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`oc tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `oc tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `oc tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `oc tools` → "reconfigure existing" shows
...
#    reported as added/removed by `oc tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `oc tools` printed ``- kanban``
