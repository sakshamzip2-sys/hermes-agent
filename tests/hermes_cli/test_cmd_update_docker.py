"""Tests for ``oc update`` / ``--check`` inside the Docker container.
...
fix, ``oc update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side OpenComputer, not an update to the running container, so the message
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``oc update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``oc update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
