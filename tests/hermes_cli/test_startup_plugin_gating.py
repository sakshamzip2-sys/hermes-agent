...
This saves 500-650ms on ``oc --help``, ``oc version``,
``oc logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``oc --help`` in-process and parse the subcommand block.
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``oc help <cmd>`` skips discovery; it won't show
