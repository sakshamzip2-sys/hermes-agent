"""Tests for `oc memory setup [provider]` routing.
...
``oc memory setup honcho``) without the interactive picker — which matters
because the per-provider ``oc <provider>`` subcommand is only registered
