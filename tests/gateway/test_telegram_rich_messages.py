...
async def test_rich_messages_default_is_enabled():
    """Fork default: rich messages are ON by default (Bot API 10.1); rich-eligible
    content (tables/task lists/details/math) goes through sendRichMessage without
    the user having to opt in. Set telegram.extra.rich_messages: false for legacy."""
...
async def test_expect_edits_metadata_keeps_preview_on_legacy_path():
    ...
    # rich messages until OpenComputer wires rich_message edits directly.
    bot = adapter._bot
    assert bot is not None
    bot.do_api_request.assert_not_called()
    bot.send_message.assert_awaited()
