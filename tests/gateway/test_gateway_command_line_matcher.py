...
Regression guard for the Windows ``oc gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    "hermes-gateway.exe",
    "oc gateway",          # bare `oc gateway` defaults to run
    "oc gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "oc gateway --profile work run",
    "python -m hermes_cli.main gateway -p work run",
    "oc gateway --profile=work run",
    # a profile literally NAMED "gateway"
    "oc -p gateway gateway run",
