...
def test_cli_register_includes_node_subcommand():
    """`oc meet` argparse tree includes the node subtree."""
    import argparse
...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "OpenComputer", "OpenComputer"):
        assert not _looks_like_human_speaker(s, "OpenComputer"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "OpenComputer"), f"{s!r} SHOULD be human"
