...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `oc update` resolves the pyproject pin and
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `oc update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `oc update` downgrades the "
