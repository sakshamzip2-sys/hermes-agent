"""Tests for the cross-OpenComputer-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another OpenComputer profile's
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake OpenComputer root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    """Build a fake OpenComputer layout:
...
class TestClassifyCrossProfileTarget:
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any OpenComputer root
