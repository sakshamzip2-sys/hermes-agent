...
# Without this, standalone invocations (e.g. after `oc update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``oc tools`` config for the ``cron`` platform.
...
def _get_hermes_home() -> Path:
    """Resolve OpenComputer home dynamically while preserving test monkeypatch hooks."""
