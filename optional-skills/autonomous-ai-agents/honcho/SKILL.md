---
name: honcho
description: Configure and use Honcho memory with OpenComputer -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: OpenComputer
license: MIT
...
# Honcho Memory for OpenComputer

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every OpenComputer profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/opencomputer#running-honcho-locally-with-opencomputer

### Verify

```bash
opencomputer honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this OpenComputer profile's AI peer
...
Honcho models conversations as interactions between **peers**. OpenComputer creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this OpenComputer instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each OpenComputer run |
| `global` | Single session across all directories |

Manual override: `opencomputer honcho map my-project-name`
...
Each OpenComputer profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block opencomputer.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `opencomputer.coder` host block in `honcho.json`
...
```bash
opencomputer honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for OpenComputer when Honcho memory is active.
...
Run `opencomputer honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `opencomputer honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `opencomputer profile create <name> --clone`. For existing profiles: `opencomputer honcho sync`.
...
| Command | Description |
|---------|-------------|
| `opencomputer honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `opencomputer honcho status` | Show resolved config, connection test, peer info for active profile |
| `opencomputer honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `opencomputer honcho disable` | Disable Honcho for the active profile |
| `opencomputer honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `opencomputer honcho peers` | Show peer identities across all profiles |
| `opencomputer honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `opencomputer honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `opencomputer honcho sessions` | List known directory-to-session-name mappings |
| `opencomputer honcho map <name>` | Map current working directory to a Honcho session name |
| `opencomputer honcho identity` | Seed AI peer identity or show both peer representations |
| `opencomputer honcho sync` | Create host blocks for all OpenComputer profiles that don't have one yet |
| `opencomputer honcho migrate` | Step-by-step migration guide from OpenClaw native memory to OpenComputer + Honcho |
| `opencomputer memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `opencomputer memory status` | Show active memory provider and config |
| `opencomputer memory off` | Disable external memory provider |
