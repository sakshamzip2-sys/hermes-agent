...
opencomputer kanban watch --tenant <project-slug>

# Snapshot of the board
opencomputer kanban list --tenant <project-slug>
opencomputer kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
opencomputer kanban stats --tenant <project-slug>

# Visual dashboard (browser)
opencomputer dashboard

# Inspect a specific task (includes comments + events)
opencomputer kanban show <task-id>

# Follow a single task's event stream
opencomputer kanban tail <task-id>
```

Verify available subcommands with `opencomputer kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `opencomputer kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `opencomputer kanban block <id>` then `opencomputer kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `opencomputer kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `opencomputer kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `opencomputer profile list` (profile exists?), `opencomputer status` (gateway/dispatcher up?), and verify tenant. |
...
opencomputer kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
opencomputer kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(opencomputer kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
opencomputer kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
opencomputer kanban block <task-id>
opencomputer kanban archive <task-id>

# Diagnose what happened
opencomputer kanban show <task-id>      # task body, comments, recent events
opencomputer kanban tail <task-id>      # follow the live event stream
opencomputer kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `opencomputer kanban create` for the director
...
while true; do
    clear
    opencomputer kanban list --tenant <slug>
    echo "---"
    opencomputer kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `opencomputer kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `opencomputer profile list` and start
