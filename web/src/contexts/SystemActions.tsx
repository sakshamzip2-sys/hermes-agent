    ...
    async (action: SystemAction) => {
      ...
      try {
        if (action === "restart") {
          ...
          // Some installs cannot apply updates from inside the dashboard (e.g. a
          // Docker install where the image is immutable, so `oc update` can't
          // apply). The endpoint returns a structured {ok:false, message,
          // update_command} envelope instead of spawning the action; surface that
          // guidance rather than polling a synthetic failed action (#34347 / #36263).
          if (!resp.ok) {
