...
appropriate.  All installs go to a OpenComputer-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``oc lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the OpenComputer-owned bin staging dir for LSP servers."""
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``oc lsp status`` CLI to give users a quick
