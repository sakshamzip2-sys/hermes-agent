...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time OpenComputer starts and finds ``~/.openclaw/``.

    Points users at ``oc claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``oc claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to OpenComputer, run "
        "`oc claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `oc claw cleanup` (renames it to ~/.openclaw.pre-migration — "
