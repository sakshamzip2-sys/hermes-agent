...
Owns one Codex thread per OpenComputer session. Drives `turn/start`, consumes
...
# OpenComputer' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse OpenComputer/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per OpenComputer session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        into OpenComputer' messages shape.
        ...
        # supports rich content but OpenComputer' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into OpenComputer' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved OpenComputer' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map OpenComputer approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    OpenComputer returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort OpenComputer version string for codex's userAgent line."""
    try:
