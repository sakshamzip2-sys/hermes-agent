...
Plugins built on OpenComputer often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing OpenComputer
...
embedded slugs, no shorthands. This mirrors OpenComputer' main config
...
handles every provider, fallback chain, and per-task override OpenComputer
supports.
"""
