...
def _hermes_root_path() -> Path:
    """Resolve the OpenComputer root dir (always the parent of any profile, never per-profile)."""
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # OpenComputer control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied OpenComputer path.

    Three categories are blocked:

      * Internal OpenComputer cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global OpenComputer
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # OpenComputer root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal OpenComputer cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a OpenComputer credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the OpenComputer MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a OpenComputer MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# OpenComputer profiles are separate HERMES_HOME dirs under
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside OpenComputer scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    OpenComputer entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to OpenComputer profile {info['target_profile']!r}, but the "
...
# regardless of which OpenComputer profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner OpenComputer-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative OpenComputer state.
    ...
    Detection is path-shape-only — does not require any OpenComputer resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host OpenComputer process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative OpenComputer state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host OpenComputer "
