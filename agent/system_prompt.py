...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  OpenComputer never re-renders parts of this string mid-
    ...
    # Coding posture (base OpenComputer, any interactive coding surface in a code
    ...
    # Active-profile hint — names the OpenComputer profile the agent is running
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active OpenComputer profile: default. Other profiles (if any) live "
        ...
        stable_parts.append(
            f"Active OpenComputer profile: {active_profile}. This session reads "
    ...
    # gate (tools/permission_rules.py) regardless — this is just a nicety so the
    # model proposes instead of trying and getting blocked.  Lives in the
    # VOLATILE tier so the cacheable STABLE tier stays byte-identical whether or
    # not plan mode is on (a normal session is unchanged from before this
    # feature).  Read live; /plan invalidates the cached prompt so the toggle
    ...
    try:
        ...
        if _perm_mode(getattr(agent, "session_id", "") or "") == _MODE_PLAN:
            volatile_parts.append(_PLAN_PROMPT)
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — OpenComputer never rebuilds or reinjects parts of it
