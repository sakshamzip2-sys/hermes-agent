 ...
 * product name (`OpenComputer`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/OpenComputer'
 *
 * This is a hard failure with no obvious cause for the user — `oc desktop`
 ...
 * (the mac-unpacked OpenComputer.app bundle) and Windows (win-unpacked), so we
