 ...
 *
 * Stamps the OpenComputer icon + identity onto the packed Windows OpenComputer.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `oc desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'OpenComputer')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'OpenComputer'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); OpenComputer.exe keeps the stock Electron icon`)
  }
}
