 ...
 * the three user-facing uninstall modes to the `oc uninstall` CLI flags,
 ...
 *
 *   macOS:   …/OpenComputer.app/Contents/MacOS/OpenComputer  → …/OpenComputer.app
 *   Windows: …\OpenComputer\OpenComputer.exe                 → …\OpenComputer  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/OpenComputer.app/Contents/MacOS/OpenComputer → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/OpenComputer.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs OpenComputer.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]OpenComputer$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) return dir
    return null
  }
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  // a problem, but OpenComputer install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
