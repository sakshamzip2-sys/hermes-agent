...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'OpenComputer Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している OpenComputer バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/opencomputer などのパスプレフィックスもサポートしています。',
    ...
    providers: {
      ...
      removeExternal: (provider, command) => `${provider} は OpenComputer の外部で管理されています。${command} で削除してください。`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@opencomputer:example.org' },
  ...
  updates: {
    ...
    applyingBody: 'OpenComputer アップデーターが独自のウィンドウで引き継ぎ、完了後に OpenComputer を再度開きます。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると OpenComputer が自動的に再接続します。',
    applyingClose: 'OpenComputer は更新を適用するために閉じます。',
  ...
  onboarding: {
    ...
    flowSubtitles: {
      ...
      loopback: 'サインインのためブラウザーを開きます — OpenComputer が自動接続します',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'OpenComputer 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  desktop: {
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`opencomputer gateway` は起動していますか？'
    }
  },
