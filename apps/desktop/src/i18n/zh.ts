...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'OpenComputer Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 OpenComputer 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 OpenComputer 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      remoteUrlTitle: '远程 URL',
      remoteUrlDesc: '远程 dashboard 后端的基础 URL。支持路径前缀，例如 /opencomputer。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'OpenComputer Desktop 将使用已保存设置重新连接。',
    ...
    providers: {
      ...
      removeExternal: (provider, command) => `${provider} 由 OpenComputer 外部管理。请使用 ${command} 移除。`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 用户 ID', placeholder: '@opencomputer:example.org' },
    ...
    platformIntro: {
      ...
      weixin:
        '运行 `opencomputer gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。OpenComputer 会通过腾讯 iLink Bot API 连接并保存凭据。',
  ...
  updates: {
    ...
    applyingBody: 'OpenComputer 更新器会在自己的窗口中接管，并在完成后重新打开 OpenComputer。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 OpenComputer 会自动重新连接。',
    applyingClose: 'OpenComputer 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  onboarding: {
    ...
    flowSubtitles: {
      ...
      loopback: '打开浏览器登录 — OpenComputer 会自动连接',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayOffline: '离线',
      gatewayTitle: 'OpenComputer 推理网关状态',
      agents: '代理',
      closeAgents: '关闭代理',
      openAgents: '打开代理',
      subagents: count => `${count} 个子代理`,
  ...
  desktop: {
    ...
    handoff: {
      ...
      timedOut: '等待网关超时。`opencomputer gateway` 是否正在运行？'
    }
  },
