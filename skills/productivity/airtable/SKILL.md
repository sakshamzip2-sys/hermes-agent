...
4. Store the token in `${HERMES_HOME:-~/.hermes}/.env` (or via `opencomputer setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for OpenComputer. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for OpenComputer
