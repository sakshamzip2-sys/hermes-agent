...
## Prerequisites and install method (read first)

This skill drives a two-machine setup that continuously syncs the user's real Chrome cookies and logged-in sessions from a source Mac to a sink Mac. Treat that as sharing live credentials. Do NOT start any daemon until every item below holds and the user has explicitly confirmed.

Hard requirements (stop and ask if any is missing):
1. TWO machines, both macOS. The sink (where agents run) must be a Mac. Linux sinks are not supported in current releases, so a Linux cloud VM cannot be the sink yet.
2. Tailscale running on both, and the source can SSH to the sink without a password prompt.
3. Google Chrome installed on the sink.

Install method (prefer the signed release):
- PREFERRED: download the signed pre-built release for the machine's architecture from https://github.com/mvanhorn/agentcookie/releases (latest v0.17.1) and put `agentcookie` on PATH on BOTH machines. The signed binary takes a one-time Keychain grant at `wizard install` and never prompts again.
- FALLBACK (only with Go 1.22+ and acceptance of per-run Keychain prompts): `go install github.com/mvanhorn/agentcookie/cmd/agentcookie@latest`. That binary is unsigned, so macOS prompts for the Keychain password on every run. Go is not installed in this environment by default.

Consent gate: before pairing, confirm with the user which machine is source and which is sink, and get an explicit go to sync live cookies. Never run the source or sink wizard without that go.
...
Install the binary if missing. Prefer the signed pre-built release (see Prerequisites); use `go install` only as a Go-1.22+ fallback that prompts Keychain on every run:

```bash
# Preferred: place the signed `agentcookie` from the GitHub release on PATH, then verify:
command -v agentcookie || echo "download signed release: https://github.com/mvanhorn/agentcookie/releases"
# Fallback only (unsigned, needs Go 1.22+):
