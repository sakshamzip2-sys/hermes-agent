"""
OpenComputer MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (OpenComputer-specific extra)

Usage:
    oc mcp serve
    oc mcp serve --verbose
...
class EventBridge:
    ...
    This is the OpenComputer equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the OpenComputer MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "OpenComputer messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the OpenComputer MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
